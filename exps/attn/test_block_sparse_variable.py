...
def get_sdpa_attn_ref(
    q, k, v, grad_output, block_mask, block_row_sz, block_col_sz, high_precision=False
):
    ...
    sdpa_mask_4d = get_sdpa_mask_from_var_block_mask(
        block_mask, block_row_sz, block_col_sz, SEQLEN, SEQLEN, BATCH_SIZE
    )
...
def assert_close_to_torch_ref(
    ...
    high_precision_torch_out_ref = get_sdpa_attn_ref(
        q, k, v, grad_output, block_mask, block_row_sz, block_col_sz, True
    )
    ...
    low_precision_torch_out_ref = get_sdpa_attn_ref(
        q, k, v, grad_output, block_mask, block_row_sz, block_col_sz
    )
...
def test_q_headwise_sparse():
    ...
    assert_close_to_torch_ref(
        q, k, v, grad_output, block_mask, block_row_sz, block_col_sz, "q"
    )
