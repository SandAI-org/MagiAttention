...
from ._fsdp_common import MagiFSDPMeshInfo, MagiHSDPMeshInfo, _is_composable_with_fsdp
...
def _get_post_forward_mesh_info(
    reshard_after_forward: Union[bool, int], mesh_info: MagiFSDPMeshInfo
) -> Optional[MagiFSDPMeshInfo]:
    ...
    if reshard_after_forward is True:
        ...
        post_forward_mesh_info = MagiHSDPMeshInfo(
...
def _move_states_to_device(
    ...
    """
    We have MagiFSDP move states to device for simpler and faster initialization
    since MagiFSDP almost always uses CUDA for training. We move parameters/buffers
    ...
    for tensor in itertools.chain(params, buffers):
        ...
        if isinstance(tensor, DTensor):
            if (dtensor_mesh_type := tensor.device_mesh.device_type) != device.type:
                raise ValueError(
                    "Requires DTensor to have mesh of the same type as the MagiFSDP mesh "
                    f"but got {dtensor_mesh_type} for DTensor and {device.type} for MagiFSDP"
                )
