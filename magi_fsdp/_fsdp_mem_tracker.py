...
class MagiFSDPMemTracker(MemTracker):
    """
    ...
    .. code-block:: python
        ...
        optimizer = ...
        inp = ...
        fmt = MagiFSDPMemTracker(module, optimizer)
    ...
    def __init__(
        ...
        assert isinstance(
            mod, MagiFSDPModule
        ), "MagiFSDPMemTracker only supports FSDP modules"
        self._root_mod = mod
        self._optm = optm
    ...
    def _instrument_fsdp_sharded_params_grads(
        self, fsdp_param_group: MagiFSDPParamGroup
    ) -> None:
        ...
        def inner(
            *args: _P.args, **kwargs: _P.kwargs
            ...
            if fsdp_mod not in self.memory_tracking:
                ...
                mod_stat.snapshots.setdefault(_FSDPModState.PEAK_FW, []).append(  # type: ignore[arg-type]
                    snapshot
                )
                mod_stat.snapshots.setdefault(_FSDPModState.BEF_PRE_FW, []).append(  # type: ignore[arg-type]
                    deepcopy(snapshot)
                )
                ...
                if len(parents) == 1 and "Global" in parents:
                    raise NotImplementedError(
                        "MagiFSDPMemTracker does not support memory tracking for multiple iterative calls."
            ...
            if self._mod_tracker.is_bw:
                ...
                if self._ac_mod is None:  # type: ignore[has-type]
                    self._ac_mod = weakref.ref(fsdp_mod)
                    self._in_ac = True
            else:
                state = _FSDPModState.AFT_PRE_FW
            mod_stat.snapshots.setdefault(state, []).append(self.get_tracker_snapshot())  # type: ignore[arg-type]
    ...
    def _fsdp_param_group_pre_backward(
        ...
        # We capture memory snapshots before and after ``MagiFSDPParamGroup.pre_backward`` to capture the pre-fetching
    ...
    def _fsdp_param_group_post_backward(
        ...
        # We capture the memory snapshots before and after ``MagiFSDPParamGroup.post_backward`` to track and attribute
    ...
    def _instrument_fsdp_module(self) -> None:
        ...
        # our own hooks that wrap them. We choose this over monkey-patching `MagiFSDPParamGroup.pre_forward` and
        # `MagiFSDPParamGroup.post_forward` because during AC these won't be called.
        # TODO(@sanketpurandare): This will need to be modified after this PR (https://github.com/pytorch/pytorch/pull/127786)
        # lands. For backward we monkey-patch the `MagiFSDPParamGroup.pre_backward` and `MagiFSDPParamGroup.post_backward`.
        for module in self._root_mod.modules():  # type: ignore[attr-defined]
            if isinstance(module, MagiFSDPModule):
                ...
                if fsdp_param_group := fsdp_state._fsdp_param_group:
                    ...
                    fsdp_state._pre_forward_hook_handle = (
                        module.register_forward_pre_hook(  # type: ignore[attr-defined]
                    ...
                    fsdp_state._post_forward_hook_handle = module.register_forward_hook(  # type: ignore[attr-defined]
        ...
        for buffer in self._root_mod.buffers():  # type: ignore[attr-defined]
            self._update_and_maybe_create_winfos(
                buffer,
                _FSDPRefType.BUFFER,
            )
    ...
    def _deregister_module_and_optimizer_hooks(self) -> None:
        ...
        if self._optimizer_hook_handles is not None:
            for handle in self._optimizer_hook_handles:
                handle.remove()
            self._optimizer_hook_handles = None  # type: ignore[assignment]
    ...
    def track_external(
        ...
        """This is no-op for ``MagiFSDPMemTracker``"""

    def __enter__(self) -> "MagiFSDPMemTracker":
        if self._depth == 0:
