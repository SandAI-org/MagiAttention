...
from magi_fsdp._fsdp_param_group import RegisterPostBackwardFunction
...
def _assert_module_states(
    ...
    for state in olist[1:]:  # type: ignore[unreachable]
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.buffer is not None:
            z = z + self.buffer  # type: ignore[unreachable]
        return z
