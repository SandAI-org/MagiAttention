...
from ._fsdp_param_group import MagiFSDPCommContext, MagiFSDPParamGroup

if TYPE_CHECKING:
    from ._fsdp_param import MagiFSDPParam


logger = logging.getLogger("magi_fsdp")


class MagiFSDPStateContext:
    """This has state shared across MagiFSDP states."""

    def __init__(self) -> None:
        # All FSDP states in the root state's module tree
        self.all_states: list[MagiFSDPState] = []
        # Iteration's forward root runs the once-per-forward logic; this root
        # may not be the overall root set by lazy initialization in cases where
        # only a submodule runs forward (e.g. encoder-only for eval)
        self.iter_forward_root: Optional[MagiFSDPState] = None
...
class MagiFSDPState(_State):
    def __init__(self) -> None:
        super().__init__()
        self._fsdp_param_group: Optional[MagiFSDPParamGroup] = None
    ...
    def _root_pre_forward(
        ...
        if not compiled_autograd_enabled():
            logger.debug("MagiFSDP::root_pre_forward")
        self._state_ctx.iter_forward_root = self
        with torch.profiler.record_function("MagiFSDP::root_pre_forward"):
            ...
            if self._device.type in ["cuda", "hpu", "xpu", "mtia"]:
                with torch.profiler.record_function("MagiFSDP::inputs_to_device"):
    ...
    def _lazy_init(self) -> None:
        ...
        finalized (e.g. MagiFSDP has been applied to all desired modules). This
        ...
        if len(self._modules) > 1:
            raise RuntimeError(
                f"MagiFSDP requires a single root module but got {self._modules}"
            )
        detect_compiled_autograd()
        root_module = self._modules[0]
        visited_states: set[MagiFSDPState] = set()
        for module_name, module in root_module.named_modules():
            ...
            if module is not root_module:
                if state not in visited_states and state._is_root is not None:
                    raise RuntimeError(
                        "MagiFSDP state has already been lazily initialized for "
                        f"{module_name}\nMagiFSDP requires running forward through "
    ...
    def _init_fqns(self) -> None:
        ...
        root_module = self._modules[0]
        param_to_fsdp_param: dict[nn.Parameter, MagiFSDPParam] = {}
        module_to_fsdp_param_group: dict[nn.Module, MagiFSDPParamGroup] = {}
        for state in self._state_ctx.all_states:
            if fsdp_param_group := state._fsdp_param_group:
    ...
    def _pre_forward(
        ...
        if self._mp_policy.cast_forward_inputs and self._mp_policy.param_dtype:
            with torch.profiler.record_function("MagiFSDP::cast_forward_inputs"):
        ...
        for fsdp_state in self._states_to_forward_prefetch:
            if (target_param_group := fsdp_state._fsdp_param_group) is not None:
                MagiFSDPParamGroup._prefetch_unshard(target_param_group, "forward")
        return args, kwargs
    ...
    def _post_forward(self, module: nn.Module, input: Any, output: Any) -> Any:
        ...
        if self._mp_policy.output_dtype is not None:
            with torch.profiler.record_function("MagiFSDP::cast_forward_outputs"):
                output = tree_map(
    ...
    def _finalize_backward(self) -> None:
        if self._modules_to_run_forward:
            msg = (
                ...
                "which is error-prone since MagiFSDP post-forward/pre-backward logic "
...
def _get_module_fsdp_state(module: nn.Module) -> Optional[MagiFSDPState]:
    ...
    if isinstance(state, MagiFSDPState):
        return state
    return None
