...
a reference to the current all-gather's output and have the next MagiFSDP parameter
group free it after its copy-in. Finally, we have the last MagiFSDP state flush the
reference to avoid holding onto memory after forward.
"""


class MagiFSDPCommContext:
    """This has the communication state shared across MagiFSDP states/parameter groups."""
    ...
    def lazy_init(self, device: torch.device):
        ...
        self.post_forward_order: list[MagiFSDPParamGroup] = []  # will cause ref cycles
...
class MagiFSDPParamGroup:
    ...
    def __init__(
        ...
        mesh_info: MagiFSDPMeshInfo,
        post_forward_mesh_info: Optional[MagiFSDPMeshInfo],
        device: torch.device,
        ...
        if (
            ...
            warnings.warn(
                "The parameter offload feature is legacy and has not been refactored yet. "
                "It blocks the CPU and causes performance degradation.",
                UserWarning,
            )
        ...
        self.comm_ctx = MagiFSDPCommContext()
    ...
    def wait_for_unshard(self):
        ...
        with record_function(self._with_fqn("MagiFSDP::all_gather_copy_out")):
            foreach_all_gather_copy_out(
    ...
    def pre_forward(
        ...
        if not compiled_autograd_enabled():
            logger.debug("%s", self._with_fqn("MagiFSDP::pre_forward"))
        with record_function(self._with_fqn("MagiFSDP::pre_forward")):
    ...
    def post_forward(self, module: nn.Module, input: Any, output: Any):
        if not compiled_autograd_enabled():
            logger.debug("%s", self._with_fqn("MagiFSDP::post_forward"))
        with record_function(self._with_fqn("MagiFSDP::post_forward")):
            self.reshard()
    ...
    def pre_backward(self, default_prefetch: bool, *unused: Any):
        if (
            ...
            # Traceable MagiFSDP cannot trigger the param group's `post_backward` immediately after param usage;
            # instead it relies on this to trigger the previously unexecuted `post_backward`.
            self.post_backward()
        if self._training_state == TrainingState.PRE_BACKWARD:
            return
        if not compiled_autograd_enabled():
            logger.debug("%s", self._with_fqn("MagiFSDP::pre_backward"))
        with record_function(self._with_fqn("MagiFSDP::pre_backward")):
    ...
    def post_backward(self, *unused: Any):
        ...
        # MagiFSDP parameter group was not used in backward (should be a no-op)
        if not compiled_autograd_enabled():
            logger.debug("%s", self._with_fqn("MagiFSDP::post_backward"))
        self._training_state = TrainingState.POST_BACKWARD
        with record_function(self._with_fqn("MagiFSDP::post_backward_accumulate")):
            for fsdp_param in self.fsdp_params:
                fsdp_param.accumulate_unsharded_grad_if_needed()
        with record_function(self._with_fqn("MagiFSDP::post_backward_reshard")):
            ...
            fsdp_params_with_grad: list[MagiFSDPParam] = []
        ...
        if len(fsdp_params_with_grad) == 0:
            return
        with record_function(self._with_fqn("MagiFSDP::post_backward_reduce")):
            if (
    ...
    def _prefetch_unshard(
        target_fsdp_param_group: "MagiFSDPParamGroup", pass_type: str
    ) -> None:
        ...
        with (
            record_function(f"MagiFSDP::{pass_type}_prefetch for {target_fqn}"),
    ...
    def _register_post_backward_hook(
        ...
        # Traceable MagiFSDP relies on `root_post_backward_callback` to call each
        # `MagiFSDPParamGroup.post_backward`
    ...
    def _is_hsdp(self) -> bool:
        return isinstance(self.mesh_info, MagiHSDPMeshInfo)

    @property
    def _all_gather_process_group(self) -> dist.ProcessGroup:
        mesh_info = (
            cast(MagiFSDPMeshInfo, self.post_forward_mesh_info)
            if self.is_sharded_post_forward
            else self.mesh_info
        )
        assert isinstance(mesh_info, MagiFSDPMeshInfo)
        return mesh_info.shard_process_group

    @property
    def _reduce_scatter_process_group(self) -> dist.ProcessGroup:
        assert isinstance(self.mesh_info, MagiFSDPMeshInfo)
        return self.mesh_info.shard_process_group

    @property
    def _all_reduce_process_group(self) -> dist.ProcessGroup:
        assert isinstance(self.mesh_info, MagiHSDPMeshInfo)
    ...
    def __repr__(self):
        return f"MagiFSDPParamGroup(fqn={self._module_fqn})"
    ...
    def _validate_no_meta_params(self):
        ...
        if param_names_on_meta:
            raise RuntimeError(
                "MagiFSDP parameters should be materialized from meta device before training, "
        ...
        if ema_param_names_on_meta:
            raise RuntimeError(
                ...
                f"but the following were still on meta device: {ema_param_names_on_meta}\n"
            )

    def _validate_cpu_offload_params(self):
        if not (
            isinstance(self.offload_policy, CPUOffloadPolicy)
            and self.offload_policy.offload_param
        ):
            return
        ...
        if fsdp_params_not_on_cpu:
            raise RuntimeError(
                "MagiFSDP parameters should be materialized on CPU when enabling CPU offloading. "
...
class RegisterPostBackwardFunction(torch.autograd.Function):
    ...
    def _assert_not_tracing_fsdp():
        if compiled_autograd_enabled():
            # TODO: Find a way to print the offending MagiFSDP module.
            msg = """\
When Traceable MagiFSDP is enabled, we should not be calling into `RegisterPostBackwardFunction`.
Instead, we rely on the param group's next `pre_backward` hook to trigger its previously unexecuted
`post_backward`, and we rely on MagiFSDPState's `root_post_backward_callback` to trigger the resharding
of any leftover unsharded param groups.
If you are here, it means the forward part of this MagiFSDP instance is not compiled, and you must also
compile the forward part if you want to use Traceable MagiFSDP."""
            torch._dynamo.comptime.comptime.print(msg)
            raise RuntimeError(msg)

    @staticmethod
    def forward(ctx, param_group: MagiFSDPParamGroup, *inputs: torch.Tensor):
