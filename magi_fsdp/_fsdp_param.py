...
"""
[Note: MagiFSDP tensors]
MagiFSDP considers the following tensors:
- Original parameter: parameter passed to :class:`MagiFSDPParam`, i.e. the one
  on the module when applying MagiFSDP
...
[Note: MagiFSDP and autograd]
MagiFSDP dynamically frees and allocates the unsharded parameter. Since autograd
...
"""
[Note: Avoiding functionalization for magi_fsdp.copy_ and inductor.resize_storage_bytes_]

Currently we don't functionalize `magi_fsdp.copy_` op or `inductor.resize_storage_bytes_` op
(i.e. they show up as a mutation op in the middle of the AOT joint graph).

Reason:
Traceable MagiFSDP compiled autograd BWD graph have the following traits:
...
So this doesn't work at all for Traceable MagiFSDP.

The compromise we use is to avoid functionalization for the MagiFSDP copy_ and resize_ ops.
...
it is now more feasible to functionalize all of the mutable MagiFSDP ops. Some of the pros and cons are:
...
But maybe there are few enough mutations induced by MagiFSDP for this to matter.
"""
...
class MagiFSDPParam:
    """
    This class manages a parameter with MagiFSDP or MagiFSDP variants applied,
    ...
    def _init_sharded_param(
        ...
        if not param.is_contiguous():
            raise NotImplementedError(
                f"MagiFSDP does not support non-contiguous parameters yet: {param.shape=} {param.stride()=}"
            )
        ...
        if self.is_dtensor:
            ...
            if dp_global_mesh != tp_global_mesh or (
                ...
                raise AssertionError(
                    "MagiFSDP requires the DP and TP mesh to have the same parent mesh but got: \n"
                    f"DP's global mesh: {dp_global_mesh}\nTP's global mesh: {tp_global_mesh}"
                )
            name_dims_error = (
                "MagiFSDP requires named DeviceMesh dims for ND parallelism"
            )
            ...
            if len(self._tp_spec.placements) != 1:
                raise NotImplementedError(
                    f"MagiFSDP only supports 1D TP, not {self._tp_spec.placements}"
                )
            ...
            # TODO: Enable uneven sharding for MagiFSDP+TP.
            if split_factor > 1:  # MagiFSDP has strided sharding on tensor dim 0
                ...
                if tensor_size_dim_0 % num_shards != 0:
                    raise NotImplementedError(
                        "MagiFSDP+TP sharding does not support uneven sharding for now: "
            ...
            if isinstance(self.mesh_info, MagiHSDPMeshInfo):
        ...
        if shard_dim > 0 and param_data.size(shard_dim) % shard_world_size != 0:
            ...
            # the uneven sharding (1) requires extra copies before/after MagiFSDP
            # collectives and (2) introduces extra complexity to handle padding
            # and unpadding
            raise NotImplementedError(
                f"MagiFSDP does not support uneven sharding on dim {shard_dim}: "
    ...
    def init_unsharded_param(self):
        """
        [Note: Invariants for torch.compile Traceable MagiFSDP]
    ...
    def to_sharded_post_forward_dtensor(self, tensor: torch.Tensor) -> DTensor:
        ...
        assert isinstance(self.post_forward_mesh_info, MagiHSDPMeshInfo)
    ...
    def free_unsharded_param(self) -> None:
        if compiled_autograd_enabled():
            ...
            in `init_unsharded_param()`. (For full-graph MagiFSDP case, we will then remove
            ...
            if not compiled_autograd_enabled() and hasattr(
                ...
                if num_fn_params == 1:
                    ...
                    if (
                        ...
                        raise AssertionError(
                            "When a parameter is unevenly sharded by MagiFSDP "
                            f"(orig size={self._orig_size}, MagiFSDP world size={self.mesh_info.mesh.size()}), "
...
# CPU overhead, if the module did not override it. For MagiFSDP, we know we do not
