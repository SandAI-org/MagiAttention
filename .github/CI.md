...
The package-owned H100 profile tests the canonical backend names `ffa` and `sdpa`, preserving the existing Hopper coverage. The B300 profile instead tests `fa4`, `cutedsl`, and `sdpa`; `fa4` is the test enum value for the FFA_FA4 implementation. Before a B300 portable test actually runs, the package-owned test recipe installs `flash_attn_cute` and its native utilities from the exact checked-out MagiAttention source for SM100. A runner-local content-addressed stamp shares that installation between the main and extensions test invocations in the same job. Portable hits skip both the tests and this setup. The H100 build performs the normal FFA AOT prebuild once. Unsupported parameter combinations are filtered by the test suite. The H100 profile alone retains `NCCL_NVLS_ENABLE=0`; B300 does not inherit that Hopper-specific workaround. The Dispatcher creates each task directly from `registry.cn-sh-01.sensecore.cn/sandai-ccr/magi-base:26.05.2`, matching `.github/configs/base_image_tag.txt`; the workflow must not declare a nested GitHub Actions container because task runners do not provide Docker. This image provides the remaining environment required by both MagiAttention and MagiAttnExtensions; a plain NGC PyTorch image is insufficient. Shared storage is available at `/home/niubility2/ci_workspace`.
...
The approval is the current security boundary: reviewers must inspect untrusted changes before allowing arbitrary PR code to execute on an internal runner. Approved fork PRs set `CI_WORKSPACE_ROOT` to `${RUNNER_TEMP}/magi-attention-ci`; they do not publish portable markers or intentionally write normal CI artifacts to the shared cache. For every event, the H100 build job transfers its two wheels and manifests to platform test jobs through a run-scoped GitHub Actions artifact, preserving the single-wheel contract without assuming shared storage across platforms. The current Dispatcher pool still exposes the shared mount to the task, so reviewer approval must be treated as authorization for the fork code to access internal runner resources.
