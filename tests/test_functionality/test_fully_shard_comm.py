...
from magi_fsdp import MagiFSDPModule as FSDPModule
from magi_fsdp import MixedPrecisionPolicy, OffloadPolicy, fully_shard
from magi_fsdp._fsdp_collectives import (
    _div_if_needed,
...
from magi_fsdp._fsdp_common import MagiFSDPMeshInfo as FSDPMeshInfo
from magi_fsdp._fsdp_common import TrainingState
from magi_fsdp._fsdp_init import (
    _get_post_forward_mesh_info,
    _init_default_fully_shard_mesh,
)
from magi_fsdp._fsdp_param import ShardedState
...
class TestFullyShardCollectiveOps(FSDPTestMultiThread):
    ...
    def test_all_gather_fp32(self):
        ...
        for async_op, streams, reshard_after_forward in itertools.product(
            ...
            if (
                not isinstance(reshard_after_forward, bool)
                and isinstance(reshard_after_forward, int)
                and (async_op or all_gather_stream is default_stream)
            ):
                continue
...
class TestFullyShardCommunication(FSDPTest):
    ...
    def test_set_reduce_scatter_divide_factor(self):
        self.run_subtests(
            # FIXME: self.world_size * 2 results in a small diff
