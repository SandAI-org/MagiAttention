    ...
    for dv, attention_chunk in itertools.product(dv_vals, attention_chunk_vals):
        ...
        for pack_gqa, num_splits in itertools.product(pack_gqa_vals, num_splits_vals):
            ...
            assert_close(
                ...
                test_case=f"{dv=}, {pack_gqa=}, {num_splits=} => fwd output",
            )
    ...
    for dv, attention_chunk in itertools.product(dv_vals, attention_chunk_vals):
        ...
        for pack_gqa, num_splits in itertools.product(pack_gqa_vals, num_splits_vals):
            ...
            assert_close(
                out_cmp,
                out_ref_cmp,
                atol=fwd_atol + rtol * (out_pt_cmp - out_ref_cmp).abs().max().item(),
                rtol=0,
                mismatch_threshold=1e-5,
                test_case=f"{dv=}, {pack_gqa=}, {num_splits=} => fwd output",
            )
        ...
        for num_splits, precompute_metadata in itertools.product(
            ...
            for _ in range(1 if not precompute_metadata else 2):
                ...
                assert_close(
                    out,
                    out_ref,
                    atol=1e-5 + mult * (out_pt - out_ref).abs().max().item(),
                    rtol=0,
                    mismatch_threshold=1e-5,
                    test_case=f"{dv=}, {num_splits=} => Out",
                )
...
def test_flash_attn_lse_grad(seqlen_q, seqlen_k, d, causal, dtype):
    ...
    try:
        assert_close(
            ...
            rtol=0,
            mismatch_threshold=1e-5,
            test_case=f"{seqlen_q=}, {seqlen_k=} => dQ",
        )
    except AssertionError as e:
        bwd_errors.append(str(e))
    try:
        assert_close(
            ...
            rtol=0,
            mismatch_threshold=1e-5,
            test_case=f"{seqlen_q=}, {seqlen_k=} => dK",
        )
    except AssertionError as e:
        bwd_errors.append(str(e))
    try:
        assert_close(
            ...
            rtol=0,
            mismatch_threshold=1e-5,
            test_case=f"{seqlen_q=}, {seqlen_k=} => dV",
        )
    except AssertionError as e:
        bwd_errors.append(str(e))
    if bwd_errors:
...
def test_flash_attn_paged_hd256_sm100_tma(seqlen_q):
    ...
    assert_close(
        ...
        rtol=1e-3,
        mismatch_threshold=1e-5,
        test_case=f"{seqlen_q=} => paged vs non-paged",
    )
...
def test_flash_attn_paged_hd256_sm100_tma_gqa(nheads_kv):
    ...
    assert_close(
        ...
        rtol=1e-3,
        mismatch_threshold=1e-5,
        test_case=f"{nheads_kv=} => paged GQA",
    )
...
def test_flash_attn_paged_hd256_sm100_tma_shuffled():
    ...
    assert_close(
        ...
        rtol=1e-3,
        mismatch_threshold=1e-5,
        test_case="shuffled => paged vs non-paged",
    )
    ...
    for dv, attention_chunk in itertools.product(dv_vals, attention_chunk_vals):
        ...
        for pack_gqa, num_splits in itertools.product(pack_gqa_vals, num_splits_vals):
            ...
            assert_close(
                ...
                rtol=0,
                mismatch_threshold=1e-5,
                test_case=f"{dv=}, {pack_gqa=}, {num_splits=} => Out",
            )
