...
# Siyu Wang, Shengbin Di, Yuxi Chi, Johnsonms, Linfeng Zheng, Haoyan Huang, Lanbo Li,
# Yun Zhong, Man Yuan, Minmin Sun, Yong Li, Wei Lin.
...
from magi_attention.kernel.cutedsl.testing import (
    attention_ref,
    generate_qkv,
...
# torch FakeTensorMode would enable fast cutedsl kernel compilation
# without allocating the actual GPU memory or running the kernel
    ...
    for dv, attention_chunk in itertools.product(dv_vals, attention_chunk_vals):
        ...
        if (
            ...
            if VERBOSE:
                ...
                print(
                    f"  at coordinates {tuple(c.item() for c in coords)}: "
                    f"dQ={dq[coords].item()}, dQ_ref={dq_ref[coords].item()}"
                )
                ...
                print(
                    f"  at coordinates {tuple(c.item() for c in coords)}: "
                    f"dK={dk[coords].item()}, dK_ref={dk_ref[coords].item()}"
                )
                ...
                print(
                    f"  at coordinates {tuple(c.item() for c in coords)}: "
                    f"dV={dv[coords].item()}, dV_ref={dv_ref[coords].item()}"
                )
...
def test_flash_attn_varlen_output(
    ...
    if d == 256 and IS_SM100:
        ...
        if not unpad_q or not unpad_kv:
            pytest.skip(
                "SM100 head_dim=256 2CTA kernel does not support seqused_q/seqused_k mode yet "
                "(requires unpad_q=True and unpad_kv=True)"
            )
    if (
        causal or local
...
def test_flash_attn_bwd_preallocated_outputs(seqlen_q, seqlen_k, d, causal, dtype):
    from magi_attention.kernel.cutedsl.interface import _flash_attn_bwd, _flash_attn_fwd
...
def test_flash_attn_lse_grad(seqlen_q, seqlen_k, d, causal, dtype):
    ...
    atol = 0.02
    assert (dq.float() - dq_ref).abs().max().item() <= atol, "dQ error too large"
    assert (dk.float() - dk_ref).abs().max().item() <= atol, "dK error too large"
    assert (dv.float() - dv_ref).abs().max().item() <= atol, "dV error too large"
...
def test_flash_attn_paged_hd256_sm100_tma(seqlen_q):
    ...
    assert torch.allclose(
        out_paged_0, out_ref, atol=1e-3, rtol=1e-3
...
def test_flash_attn_paged_hd256_sm100_tma_gqa(nheads_kv):
    ...
    assert torch.allclose(
        out_paged, out_ref, atol=1e-3, rtol=1e-3
...
def test_flash_attn_seqlen_k_zero(seqlen_q, d, causal):
    ...
    assert torch.all(
        out == 0
    ).item(), f"Expected all-zero output when seqlen_k=0, got max={out.abs().max().item():.6f}"
    if lse is not None:
        assert torch.all(
            torch.isinf(lse) & (lse < 0)
        ).item(), f"Expected all -inf LSE when seqlen_k=0, got: {lse}"
