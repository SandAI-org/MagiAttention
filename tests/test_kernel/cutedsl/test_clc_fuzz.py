...
from magi_attention.kernel.cutedsl import utils as cute_utils
from magi_attention.kernel.cutedsl.flash_fwd_sm100 import FlashAttentionForwardSm100
...
def check_output(
    ...
    *,
    ...
    out, _ = flash_attn_func(
        q, k, v, causal=causal, window_size=window_size, num_splits=num_splits
    )
    ...
    if assert_clc and _captured_schedulers:
        ...
        if causal or is_local:
            assert (
                sched_cls is SingleTileLPTScheduler
            ), f"Expected SingleTileLPTScheduler, got {sched_cls.__name__}"
            assert (
                sched_mode == SchedulingMode.CLC
            ), f"Expected CLC scheduling mode, got {sched_mode!r}"
            if assert_2cta:
                assert use_2cta, "Expected use_2cta_instrs=True but got False"
        else:
            assert (
                sched_cls is StaticPersistentTileScheduler
            ), f"Expected StaticPersistentTileScheduler for dense noncausal, got {sched_cls.__name__}"
            assert (
                sched_mode == SchedulingMode.STATIC
...
class TestCLCNonAlignedShapes:
    @pytest.mark.parametrize(
        "sq", [1, 3, 7, 15, 31, 33, 63, 65, 127, 129, 191, 193, 255, 257]
    )
...
class TestCLCHeadDim:
    ...
    def test_overlap_sO_sQ_fallback(self):
        from magi_attention.kernel.cutedsl.tile_scheduler import SingleTileScheduler
        ...
        assert (
            sched_cls is SingleTileScheduler
        ), f"Expected SingleTileScheduler fallback, got {sched_cls.__name__}"
        assert (
            sched_mode == SchedulingMode.STATIC
        ), f"Expected STATIC fallback, got {sched_mode!r}"
...
class TestCLCFallback:
    def test_varlen_mha_uses_static(self):
        ...
        cu_seqlens = torch.cat([torch.zeros(1, dtype=torch.int32), lens.cumsum(0)]).to(
        ...
        sched_cls, sched_mode, *_ = _captured_schedulers[-1]
        assert (
            sched_cls is SingleTileVarlenScheduler
        ), f"Expected SingleTileVarlenScheduler for varlen, got {sched_cls.__name__}"
        assert (
            sched_mode == SchedulingMode.STATIC
        ), f"Expected STATIC scheduling mode, got {sched_mode!r}"
...
def check_varlen_output(
    seqlens, heads, d, *, causal=False, kv_heads=None, num_splits=1
):
    kv_heads = kv_heads or heads
    cu_seqlens = torch.cat(
        [
            torch.zeros(1, dtype=torch.int32),
            torch.tensor(seqlens, dtype=torch.int32).cumsum(0),
        ]
    ...
    if _captured_schedulers:
        ...
        assert (
            sched_cls is SingleTileVarlenScheduler
        ), f"Expected SingleTileVarlenScheduler, got {sched_cls.__name__}"
        expected_sched_mode = (
            SchedulingMode.CLC if heads != kv_heads else SchedulingMode.STATIC
        )
        assert (
            sched_mode == expected_sched_mode
    ...
    for i in range(len(seqlens)):
        ...
        out_pt_i, _ = attention_ref(
            qi, ki, vi, causal=causal, upcast=False, reorder_ops=True
        )
...
def check_varlen_output_seqused(
    seqlens, heads, d, *, causal=False, kv_heads=None, num_splits=1
):
    kv_heads = kv_heads or heads
    batch = len(seqlens)
    ...
    out_pt, _ = attention_ref(
        q, k, v, q_mask, k_mask, causal=causal, upcast=False, reorder_ops=True
    )
...
class TestCLCVarlen:
    ...
    def test_varlen_seqused_splitkv_gqa(self, seqlens, num_splits):
        check_varlen_output_seqused(
            seqlens, heads=8, kv_heads=2, d=64, num_splits=num_splits
        )
...
class TestCLCLargeScale:
    def test_large_batch(self):
        check_output(
            randn(32, 512, 8, 128), randn(32, 512, 8, 128), randn(32, 512, 8, 128)
        )

    def test_long_seq(self):
        check_output(
            randn(2, 4096, 4, 128), randn(2, 4096, 4, 128), randn(2, 4096, 4, 128)
        )

    def test_many_heads(self):
        check_output(
            randn(4, 512, 32, 128), randn(4, 512, 32, 128), randn(4, 512, 32, 128)
        )
    ...
    def test_work_stealing_pressure(self, batch, heads, sq, sk):
        ...
        assert (
            total_tiles > SM_COUNT
        ), f"expected total_tiles={total_tiles} > sm_count={SM_COUNT}"
        check_output(
            randn(batch, sq, heads, 128),
            randn(batch, sk, heads, 128),
            randn(batch, sk, heads, 128),
    ...
    def test_long_k_short_q(self):
        check_output(
            randn(8, 64, 8, 128), randn(8, 8192, 8, 128), randn(8, 8192, 8, 128)
        )
...
class TestCLCRepeatability:
    ...
    def test_repeat_mismatch(self, trial):
        torch.random.manual_seed(trial)
        check_output(
            randn(7, 192, 5, 128), randn(7, 513, 5, 128), randn(7, 513, 5, 128)
        )
    ...
    def test_repeat_2cta(self, trial):
        torch.random.manual_seed(trial)
        check_output(
            randn(9, 257, 3, 128), randn(9, 511, 3, 128), randn(9, 511, 3, 128)
        )
    ...
    def test_repeat_gqa_mismatch(self, trial):
        torch.random.manual_seed(trial)
        check_output(
            randn(5, 128, 8, 128), randn(5, 1024, 2, 128), randn(5, 1024, 2, 128)
        )
