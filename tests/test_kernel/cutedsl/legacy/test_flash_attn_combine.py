...
from magi_attention.kernel.cutedsl.legacy.interface import flash_attn_combine
...
def test_flash_attn_combine(num_splits, seqlen, d, dtype):
    ...
    # batch_size = 1
    # nheads = 1
    # Create tensors in the expected format:
    # (num_splits, batch_size, seqlen, nheads, d) and (num_splits, batch_size, seqlen, nheads)
...
def test_flash_attn_combine_varlen(varlen_mode, num_splits, seqlen, d, dtype):
    ...
    seqlens = torch.randint(
        1, seqlen + 1, (batch_size,), device=device, dtype=torch.int32
    )
    ...
    seqused_vals = (
        torch.clamp(
            seqlens
            - torch.randint(
                0, max(1, seqlen // 4), (batch_size,), device=device, dtype=torch.int32
            ),
            min=1,
        )
    ...
    if use_cu_seqlens:
        ...
        for i in range(batch_size):
            ...
            check_combine_results(
                out[start : start + sl],
                lse[start : start + sl],
                out_ref[start : start + sl],
                lse_ref[start : start + sl],
                dtype,
            )
