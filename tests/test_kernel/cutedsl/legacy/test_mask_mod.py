...
def compute_reference_flex_attn(
    tensors, mask_mod_flex, block_size: tuple[int, int] | None = None
):
...
def assert_fwd_matches_reference(
    out_cute, out_ref_fp32, out_pt, test_desc: str | None = None
):
...
def assert_bwd_matches_reference(
    ...
    dq_atol = max(
        bwd_atol_floor, 2 * (dq_ref_fp32 + 0.3 - 0.3 - dq_ref_fp32).abs().max().item()
    )
    dk_atol = max(
        bwd_atol_floor, 2 * (dk_ref_fp32 + 0.3 - 0.3 - dk_ref_fp32).abs().max().item()
    )
    dv_atol = max(
        bwd_atol_floor, 2 * (dv_ref_fp32 + 0.3 - 0.3 - dv_ref_fp32).abs().max().item()
    )
...
def get_coarse_block_mask_pair(sparse_tile_m: int, tile_n: int, last_block: int):
    ...
    def _cute_coarse_block_mask(
        ...
        return ((q_block == zero) & (n_block == zero)) | (
            (q_block == one) & (n_block == last)
        )

    def _flex_coarse_block_mask(b, h, q_idx, kv_idx):
        q_block = q_idx // sparse_tile_m
        n_block = kv_idx // tile_n
        return ((q_block == 0) & (n_block == 0)) | (
            (q_block == 1) & (n_block == last_block)
        )
...
SEQLEN_PAIRS_SMOKE = [(128, 128), (256, 256), (113, 203), (1024, 1024), (128, 8192)]
...
def _run_mask_test(
    ...
    if (
        ...
        and (sparse_tile_m, tile_n) != (128, 128)
    ):
...
@pytest.mark.skipif(
    COMPUTE_CAPABILITY != 10, reason="Test uses SM100 block mask conventions (2*tile_m)"
)
def test_single_doc_bwd_minimal():
    ...
    doc_ids = torch.zeros(
        batch_size, nheads, max(seqlen_q, seqlen_k), dtype=torch.int32, device="cuda"
    )

    from mask_mod_definitions import get_mask_pair

    mask_mod_cute, mask_mod_flex = get_mask_pair(
        "document", seqlen_q=seqlen_q, seqlen_k=seqlen_k
    )
...
def test_sm100_block_sparse_q_stage1():
    from magi_attention.kernel.cutedsl.legacy import flash_fwd_sm100
    from magi_attention.kernel.cutedsl.legacy.interface import _flash_attn_fwd
...
def test_sm100_block_sparse_coarse_blocks():
    ...
    out_ref = compute_reference_flex_attn(
        tensors, mask_mod_flex, (sparse_tile_m, tile_n)
    )
...
def test_sm100_block_sparse_coarse_blocks_mismatch():
    ...
    with mock.patch(
        "magi_attention.kernel.cutedsl.interface.normalize_block_sparse_config",
        wrapped_normalize,
    ):
        out_cute, _ = _flash_attn_fwd(
            q=tensors["q"],
...
def run_flex_reference_bwd(q, k, v, block_mask, grad_out, dtype=None):
    ...
    out_ref = flex_attention(
        q_ref, k_ref, v_ref, block_mask=block_mask, enable_gqa=True
    )
    dq_ref, dk_ref, dv_ref = torch.autograd.grad(
        out_ref, (q_ref, k_ref, v_ref), grad_out_ref
    )
...
def test_gqa_block_sparse_broadcast_pattern_recompilation():
    ...
    tensors = create_tensors(
        batch_size, seqlen, seqlen, nheads, nheads_kv, headdim, headdim, dtype
    )
    q, k, v = tensors["q"], tensors["k"], tensors["v"]
    grad_out = torch.randn_like(tensors["out"])
    softmax_scale = 1.0 / math.sqrt(headdim)

    def run_with_block_mask_nheads(
        block_mask_nheads: int,
        ...
        dq, dk, dv = run_cute_mask_bwd(
            ...
            grad_out,
            mask_mod_cute,
            block_sparse_mask_bwd=block_sparse_bwd,
            tile_m=tile_m,
            tile_n=tile_n,
        )
        return dq, dk, dv
    ...
    _, dq_ref, dk_ref, dv_ref = run_flex_reference_bwd(
        q, k, v, flex_block_mask, grad_out, dtype=torch.float32
    )
    ...
    dq_no_broadcast, dk_no_broadcast, dv_no_broadcast = run_with_block_mask_nheads(
        nheads
    )
    ...
    assert (
        err_no_broadcast_dq < 0.1
    ), f"No-broadcast dQ error too large: {err_no_broadcast_dq:.2e}"
...
def test_gqa_expand_stride_zero_bug():
    ...
    k_orig = torch.randn(
        batch_size, seqlen, n_kv_heads, headdim, device=device, dtype=dtype
    )
    v_orig = torch.randn(
        batch_size, seqlen, n_kv_heads, headdim, device=device, dtype=dtype
    )
...
@pytest.mark.skipif(
    COMPUTE_CAPABILITY not in (10, 11), reason="SM100/SM110 persistent forward only"
)
def test_persistent_blocksparse_empty_tiles():
    ...
    k = torch.randn(
        batch_size, seqlen_k, nheads_kv, headdim, device="cuda", dtype=dtype
    )
    v = torch.randn(
        batch_size, seqlen_k, nheads_kv, headdim, device="cuda", dtype=dtype
    )
    ...
    out, lse = _flash_attn_fwd(
        ...
        v=v,
        out=torch.empty(
            batch_size, seqlen_q, nheads_q, headdim, device="cuda", dtype=dtype
        ),
        lse=torch.empty(
            batch_size, nheads_q, seqlen_q, device="cuda", dtype=torch.float32
        ),
        cu_seqlens_q=None,
        cu_seqlens_k=None,
        seqused_q=None,
...
def _build_dense_from_ordered(num_blocks, indices, num_cols):
    ...
    batch_is_broadcast = B == 1 or (
        indices.stride(0) == 0 and num_blocks.stride(0) == 0
    )
                ...
                if not found and full_q_cnt is not None:
                    ...
                    for i in range(cnt_full):
                        if full_q_idx[b, h, target_n, i].item() == m:
                            assert (
                                dq_wo_full[b, h, target_n, i].item() == 0
                            ), f"n_block={target_n} (full) should get lock_value=0 for m_block={m} (spt={spt})"
                            found = True
                            break
                assert (
                    found
                ), f"target n_block={target_n} not found in backward lists for m_block={m}"
    ...
    for b in range(B):
        for h in range(H):
            for m in range(num_m):
                ...
                assert lock_vals == set(
                    range(total)
                ), f"m_block={m}: expected ranks {{0..{total - 1}}}, got {lock_vals}"


def _run_write_order_test(
    mask_mod_flex, seqlen_q, seqlen_k, block_size, B=1, H=4, spt=False
):
    bm = create_block_mask(
        mask_mod_flex,
        B,
...
@pytest.mark.skipif(
    ...
    reason="deterministic bwd only supported on sm100/sm110",
)
...
def test_block_sparse_bwd_deterministic(
    seqlen_q, seqlen_k, mask_name, window_size, spt, kv_mode
):
    torch.manual_seed(42)
    ...
    if mask_name == "document":
        doc_ids = random_doc_id_tensor(
            nheads, batch_size, max(seqlen_q, seqlen_k), device="cuda"
        ).to(torch.int32)
        original_flex_mask = mask_mod_flex
    ...
    window_size_left_arg = (
        window_size if spt and mask_name == "sliding_window" else None
    )
    ...
    out_cute, lse_cute = _flash_attn_fwd(
        ...
        v=v,
        out=torch.empty(
            batch_size, seqlen_q, nheads, headdim, device="cuda", dtype=dtype
        ),
        lse=torch.empty(
            batch_size, nheads, seqlen_q, device="cuda", dtype=torch.float32
        ),
        cu_seqlens_q=None,
        cu_seqlens_k=None,
        seqused_q=None,
...
def test_block_sparse_bwd_deterministic_missing_dq_write_order_raises():
    ...
    with pytest.raises(
        ValueError, match="requires dq_write_order in block_sparse_tensors"
    ):
        run_cute_mask_bwd(
            q,
            k,
...
def test_block_sparse_bwd_deterministic_missing_dq_write_order_full_raises():
    ...
    with pytest.raises(
        ValueError, match="requires dq_write_order_full when full blocks are present"
    ):
        run_cute_mask_bwd(
            q,
            k,
...
@pytest.mark.parametrize(
    "mask_name",
    ["block_diagonal", "mini_causal", "prefix_lm", "dilated_sliding_window"],
)
@pytest.mark.parametrize("spt", [False, True])
...
@pytest.mark.parametrize("spt", [False, True])
def test_dq_write_order_parameterized_masks(
    seqlen_q, seqlen_k, mask_name, window_size, spt
):
    torch.manual_seed(42)
    if mask_name == "sliding_window" and seqlen_q > seqlen_k:
        pytest.skip("sliding_window requires seqlen_q <= seqlen_k")
...
def test_dq_write_order_document_mask(seqlen_q, seqlen_k, spt):
    torch.manual_seed(42)
    B, H = 1, 4
    doc_ids = random_doc_id_tensor(H, B, max(seqlen_q, seqlen_k), device="cuda").to(
        torch.int32
    )
    ...
    _run_write_order_test(
        doc_mask, seqlen_q, seqlen_k, block_size=128, B=B, H=H, spt=spt
    )
...
def test_compact_block_sparse_indices():
    ...
    full_kv_idx_compact = (
        full_kv_idx[:, :, :, :max_k].contiguous() if full_kv_idx is not None else None
    )
