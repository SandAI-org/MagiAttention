...
def test_flash_attn_varlen_output(seqlen, d, causal, mha_type, dtype):
    ...
    k_ref = (
        torch.randn(batch_size, seqlen, nheads_kv, d, device=device, dtype=dtype)
        .to(dtype)
        .requires_grad_()
    )
    v_ref = (
    ...
    dq_varlen, dk_varlen, dv_varlen = torch.autograd.grad(
        out_varlen, (q_varlen, k_varlen, v_varlen), g
    )
...
@maybe_fake_tensor_mode(USE_FAKE_TENSOR)
def test_flash_attn_varlen_unpad_output(
    seqlen, d, causal, mha_type, unpad_q, unpad_kv, dtype
):
    ...
    key_padding_mask = (
        query_padding_mask
        if causal
        else generate_random_padding_mask(seqlen, batch_size, device, mode="random")
    )
