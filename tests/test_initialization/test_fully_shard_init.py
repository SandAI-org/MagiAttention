...
from magi_fsdp._fsdp_init import _get_managed_modules, _get_managed_states
from magi_fsdp._fsdp_module import MagiFSDPModule as FSDPModule
from magi_fsdp._fsdp_param import ParamModuleInfo
from magi_fsdp._fsdp_param_group import _get_param_module_infos
...
class TestFullyShardDeviceDTensor(FSDPTestMultiThread):
    ...
    def test_move_states_to_device_dtensor_invalid(self):
        ...
        regex = (
            r"Requires DTensor to have mesh of the same type as the MagiFSDP mesh "
            r"but got cpu for DTensor and cuda for MagiFSDP"
        )
...
class TestFullyShardLazyInit(FSDPTestMultiThread):
    ...
    def test_fully_shard_double_lazy_init(self):
        ...
        regex = (
            "MagiFSDP state has already been lazily initialized for 0.in_proj\n"
            "MagiFSDP requires running forward through the root module first"
        )
    ...
    def test_fully_shard_multi_module_root(self):
        ...
        regex = "MagiFSDP requires a single root module but got "
        with self.assertRaisesRegex(RuntimeError, regex):
            root_state._lazy_init()
