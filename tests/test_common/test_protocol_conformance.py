...
_ATTN_RANGE_METHODS = [
    "from_range", "clone", "offset", "truncate",
    "intersect", "intersect_size", "union", "union_size",
    "diff_by", "is_subrange_of", "is_overlap_with", "is_empty",
    "is_valid_close", "is_valid_open", "check_valid", "to_naive_range",
]

_ATTN_RANGES_METHODS = [
    "from_ranges", "from_cu_seqlens",
    "append", "insert", "extend", "pop", "clear_empty", "clone",
    "sort", "merge", "merge_with_split_alignment", "chunk", "truncate",
    "is_sorted", "is_merged", "is_non_overlap", "is_cu_seqlens",
    "is_valid", "check_valid", "is_empty",
    "to_cu_seqlens", "to_tensor", "to_naive_ranges",
    "make_range_local", "make_ranges_local",
    "find_hole_ranges", "find_overlap_ranges",
    "intersect_size", "intersect_size_with",
    "union_size", "union_size_with",
]

_ATTN_RECTANGLE_METHODS = [
    "is_valid", "check_valid", "get_valid_or_none",
    "shrink_q_range", "shrink_k_range", "shrink_d_range",
    "clone", "area",
    "cut_q", "cut_k",
    "get_rect_within_q_segment", "get_rect_within_k_segment",
    "intersection_q_id_on_left_boundary", "intersection_q_id_on_right_boundary",
    "is_full", "is_causal", "is_inv_causal", "is_bi_causal",
...
_ATTN_RECTANGLES_METHODS = [
    ...
    "append", "extend",
    "is_valid", "check_valid", "is_empty",
    "get_qo_ranges_union", "get_kv_ranges_union",
    "total_seqlen_qo", "total_seqlen_kv",
    "cut_q", "cut_k",
    "get_rects_within_q_segment", "get_rects_within_k_segment",
    "area",
]

_ATTN_MASK_TYPE_METHODS = [
    "from_int_type", "to_int_type",
]
...
class TestSignatureAlignment:
    ...
    def test_attn_range_signature(self, method_name):
        ...
        assert cpp_params is not None, f"Could not parse C++ signature for {method_name}"
        assert py_params == cpp_params, (
