...
from magi_fsdp._fsdp_common import TrainingState
from magi_fsdp._fsdp_param_group import MagiFSDPParamGroup as FSDPParamGroup
...
class TestFullyShardCompileCompute(FSDPTest):
    ...
    def test_disable_compiling_hooks(self):
        self.run_subtests(
            {
                # FIXME: Not compatible with magi_fsdp when skip_fsdp_hooks=True
    ...
    def _test_disable_compiling_hooks(
        ...
        HOOKS_FILE_NAME = "magi_fsdp/_fsdp_state.py"
...
class TestFullyShardCompile(FSDPTest):
    ...
    def _assert_no_aliased_unsharded_params_in_graph_inputs(
        ...
        storage_id_to_graph_inputs: dict[int, list] = defaultdict(list)
...
"""
        ...
        *,
        ...
        def _check_count(copy_count, resize_count):
            ...
            self.assertEqual(
                ...
                resize_count,
                (
                    f"Unexpected number of `inductor.resize_storage_bytes_` ops "
                    f"(expected {resize_count}, got {actual_resize_count}) in graph: {graph}"  # noqa: B950
                ),
            )
        ...
        for fwd_fullgraph in [True]:
            ...
            if fwd_fullgraph:
                self.assertEqual(
                    ...
                    2,
                    (
                        "Expected two separate lowerings to Triton code, "
                        "one from FWD graph and one from Compiled Autograd BWD graph"
                    ),
                )
    ...
    @unittest.skip(
        "This test results in AssertError even w/o magi_fsdp, thus skip for now"
    )
    @skipIfRocm
    @unittest.skipIf(not HAS_GPU, "Inductor+gpu needs triton and recent GPU arch")
    ...
    def test_transformer_backend_inductor_fullgraph_True(self):
        ...
        for (
            ...
            log.warning(
                f"fwd_fullgraph={fwd_fullgraph}, "
                f"all_requires_grad={all_requires_grad}, "
                f"activation_checkpoint={activation_checkpoint}"  # noqa: G004, G001, B950
            )
            with self._reinplace_all_gather_with_optional_checks(
                fwd_fullgraph
            ), torch._inductor.config.patch(
