...
class TestFullyShard1DTrainingCompose(FSDPTest):
    ...
    def _test_train_parity_with_activation_checkpointing(
        ...
        if checkpoint_impl == "wrapper":
            prefixes_to_ignore = (_CHECKPOINT_PREFIX,)  # type: ignore[assignment]
            apply_activation_checkpointing(
        ...
        if module_grouping == "mem_eff":
            ...
            fully_shard(model.layers[0], **fsdp_kwargs)  # type: ignore[call-overload]
            fully_shard([model.layers[1], model.layers[2]], **fsdp_kwargs)  # type: ignore[call-overload]
            fully_shard([model.tok_embeddings, model.pos_embeddings], **fsdp_kwargs)  # type: ignore[call-overload]
            # Embedding weights are not needed for embedding backward
            model.tok_embeddings.set_unshard_in_backward(False)
            fully_shard([model.norm, model.output], **fsdp_kwargs)  # type: ignore[call-overload]
        elif module_grouping == "mem_eff_weight_tied":
            fully_shard([model.tok_embeddings, model.output], **fsdp_kwargs)  # type: ignore[call-overload]
            for layer in model.layers:
                fully_shard(layer, **fsdp_kwargs)  # type: ignore[call-overload]
        elif module_grouping == "block":
            for layer in model.layers:
                fully_shard(layer, **fsdp_kwargs)  # type: ignore[call-overload]
        else:
            raise NotImplementedError(f"Unknown module grouping: {module_grouping}")
        fully_shard(model, **fsdp_kwargs)  # type: ignore[call-overload]
        optim = torch.optim.Adam(model.parameters(), lr=1e-2)
