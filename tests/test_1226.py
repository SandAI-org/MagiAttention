...
def true_ranges(mask: torch.Tensor) -> torch.Tensor:
    """
    给定任意形状的布尔张量，返回所有连续 True 段的 [start, end) 区间。
    返回值形状为 (k, 2)，在与输入相同的 device 上。
    """
    # 展平并转 bool
    m = mask.flatten().bool()
    if m.numel() == 0:
    ...
    padded = torch.cat(
        [
            torch.tensor([False], device=m.device),
            m,
            torch.tensor([False], device=m.device),
        ]
    )
    diff = padded[1:].int() - padded[:-1].int()
    starts = (diff == 1).nonzero(as_tuple=True)[0]  # 左闭
    ends   = (diff == -1).nonzero(as_tuple=True)[0] # 右开

    # 组合成 (k,2)；若没有 True 段则返回空
    if starts.numel() == 0:
        return torch.empty((0, 2), dtype=torch.long, device=mask.device)
...
class TestPipelineBaseWithWorldSize1(DistTestBase):
    ...
    def test_pipeline(
        ...
        for iter in range(prof_iters):
            ...
            # total_q = torch.ones_like(total_q)  # 新 tensor，仍然可参与后续计算
            # total_k = torch.ones_like(total_k)  # 新 tensor，仍然可参与后续计算
            # total_v = torch.ones_like(total_v)  # 新 tensor，仍然可参与后续计算
            # tt = total_q.dtype

            # total_q = total_q.clone()
            # total_q.fill_(2)   # 对 clone 做 in-place，不再是原来的叶子

            # total_k = total_k.clone()
            # total_k.fill_(2)   # 对 clone 做 in-place，不再是原来的叶子

            # total_v = total_v.clone()
            # total_v.fill_(2)   # 对 clone 做 in-place，不再是原来的叶子
    ...
    def _assert_close_to_torch_ref(
        ...
        print("=== q_ranges is {}, k_ranges is {}, attn_type_map is {}".format(q_ranges, k_ranges, attn_type_map))
        o_atol = EPSILON
        ...
        sub_mask = mask[7122]
        print("sub_mask shape is {}".format(sub_mask.shape))
        # torch.set_printoptions(profile="full")  # 打开完整打印
        print("sub_mask val is {}".format(sub_mask))

        print("==== true_ranges is {}".format(true_ranges(sub_mask)))
        ...
        try:
            print("==== enter fwd out close")
            print("=== total_out is {}".format(total_out))
            print("=== total_out_ref_low_precision is {}".format(total_out_ref_low_precision))
            print("=== total_out_ref_high_precision is {}".format(total_out_ref_high_precision))
            total_out_max = torch.max(total_out)      # 或 x.max()
            total_out_ref_high_precision_max = torch.max(total_out_ref_high_precision)      # 或 x.max()
            diff = total_out - total_out_ref_high_precision
            print("diff is ==== {}".format(diff))

            print("total_out_max is {}".format(total_out_max))
            print("total_out_ref_high_precision_max is {}".format(total_out_ref_high_precision_max))

            diff  = (total_out - total_out_ref_high_precision).abs()
            print(f"shape diff: {diff.shape}")  # b s h d
            print("=== total_seqlen_q is {}".format(total_seqlen_q))
            print("=== total_out shape is {}".format(total_out.shape)) #=== total_out shape is torch.Size([12288, 8, 128])
            for bs in range(1):
                for h in range(8):
                    for s in range(total_seqlen_q):
                        diff_i = diff[s, h, 0] #只取当前dim的第一个值
                        if diff_i.abs().max().item() > 1e-2:
                            out_i = total_out[s, h, 0]
                            out_ref_i = total_out_ref_high_precision[s, h, 0]
                            print(f"=== out[bs, s, h, 0] = {out_i}, out_ref[bs, s, h, 0] = {out_ref_i}, diff[bs, s, h, 0] = {diff_i}, s = {s}, h = {h}")
                        # else:
                        #     print("=== np ===")
            assert_close(
                total_out,
        ...
        if run_bwd:
            ...
            try:
                print("==== enter  grad_total_q close")
                print("=== grad_total_q is === {}".format(grad_total_q))
                print("=== grad_total_q_ref_high_precision is === {}".format(grad_total_q_ref_high_precision))
                diff_dq = grad_total_q - grad_total_q_ref_high_precision
                print("=== diff_dq is {}".format(diff_dq))
                assert_close(
                    grad_total_q,
                    grad_total_q_ref_high_precision,
                    atol=dq_atol,
            ...
            try:
                print("==== enter  grad_total_k close")
                print("=== grad_total_k is === {}".format(grad_total_k))
                print("=== grad_total_k_ref_high_precision is === {}".format(grad_total_k_ref_high_precision))
                diff_dk = grad_total_k - grad_total_k_ref_high_precision
                print("=== diff_dk is {}".format(diff_dk))
            ...
            try:
                print("==== enter  grad_total_v close")
                print("=== grad_total_v is === {}".format(grad_total_v))
                print("=== grad_total_v_ref_high_precision is === {}".format(grad_total_v_ref_high_precision))
                diff_dv = grad_total_v - grad_total_v_ref_high_precision
                print("=== diff_dv is {}".format(diff_dv))
                assert_close(
                    grad_total_v,
                    grad_total_v_ref_high_precision,
                    atol=dv_atol,
