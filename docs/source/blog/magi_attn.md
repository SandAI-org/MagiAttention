...
While the CPU scheduler controls kernel launch order to favor overlap, the GPU HyperQ driver {cite}`bradley2013hyperq` ultimately determines actual execution order non-deterministically. Therefore, with high-occupancy compute kernels like `FFA` that saturate GPU resources, ensuring actual overlap is non-trivial.
