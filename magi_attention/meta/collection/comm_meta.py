...
@dataclass(repr=False)
class CommMeta:
    # for k,v comm in fwd and dkv comm in bwd
    ...
    #   num_remote_dkv_tokens_per_stage: list[int]
    #   dkv_group_collective_args_list: list[A2AVBasedGroupCollectiveArg]
    ...
    def __post_init__(self):
        ...
        self.num_remote_dkv_tokens_per_stage: list[int] = []
        self.dkv_group_collective_args_list: list[A2AVBasedGroupCollectiveArg] = []
        ...
        for stage in range(self.overlap_degree):
            # --- for reduce dkv or maybe fetch packed kv --- #

            self.num_remote_dkv_tokens_per_stage.append(
                self.num_remote_kv_tokens_per_stage[stage] * 2
            )
            kv_group_collective_arg = self.kv_group_collective_args_list[stage]
            self.dkv_group_collective_args_list.append(
                A2AVBasedGroupCollectiveArg(
                    ...
                    packed_times=2,  # pack dkv along seqlen dim
                    reduce_op="sum",  # sum-reduce dkv
                    init_group_reduce=True,
                )
            )

            # --- for maybe fetch tupled kv --- #
            ...
            if magi_attention.comm.is_qo_comm_enable():
                # --- for fetch q, reduce dq and maybe fetch tupled q,o,do and lse  --- #
