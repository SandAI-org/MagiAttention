...
# mypy: disable-error-code="union-attr,index,attr-defined"
...
from magi_attention.kernel.cutedsl.legacy.seqlen_info import SeqlenInfo
...
class FlashAttentionForwardCombine:
    ...
    def __call__(
        ...
        if const_expr(len(mO_partial.shape) not in [4, 5]):
            raise ValueError(
                "O partial tensor must have 4 or 5 dimensions: "
                "(num_splits, batch, seqlen, nheads, headdim) or (num_splits, total_q, nheads, headdim)"
            )
        if const_expr(len(mLSE_partial.shape) not in [3, 4]):
            raise ValueError(
                "LSE partial tensor must have 3 or 4 dimensions: "
