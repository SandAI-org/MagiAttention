...
@dsl_user_op
def read_globaltimer_u64(*, loc=None, ip=None) -> cutlass.Int64:
    """Read %globaltimer (nanoseconds, cross-SM comparable)."""
...
def record_bar_probe(
    ...
    *,
    ...
    """Append one (f0, f1, f2, f3, globaltimer) record to a u64 debug buffer.

    buf (u64*): buf[0] is the record counter, atomically bumped per record.
    Record i occupies buf[1 + i*5 .. 1 + i*5 + 4] = (f0, f1, f2, f3, timer).
    Caller is responsible for keeping total records within buffer capacity.
    """
    buf_i64 = buf_ptr.toint(loc=loc, ip=ip).ir_value()
    asm = (
        "{\n"
        ".reg .b64 t;\n"
        ".reg .b64 idx;\n"
        ...
        "atom.global.add.u64 idx, [$0], 1;\n"
        "mad.lo.u64 a, idx, 40, $0;\n"
        "add.u64 a, a, 8;\n"
        "st.global.u64 [a], $1;\n"
        "st.global.u64 [a+8], $2;\n"
        "st.global.u64 [a+16], $3;\n"
        "st.global.u64 [a+24], $4;\n"
        "st.global.u64 [a+32], t;\n"
        "}"
    )
    llvm.inline_asm(
        None,
        [
            buf_i64,
            cutlass.Int64(f0).ir_value(loc=loc, ip=ip),
            cutlass.Int64(f1).ir_value(loc=loc, ip=ip),
            cutlass.Int64(f2).ir_value(loc=loc, ip=ip),
            cutlass.Int64(f3).ir_value(loc=loc, ip=ip),
        ],
        asm,
        "l,l,l,l,l",
        has_side_effects=True,
