...
class SeqlenInfo:
    ...
    offset_padded: Int32
    seqlen: Int32
    has_ranges: cutlass.Constexpr[bool] = False
    ...
    def create(
        ...
        offset = 0 if const_expr(ranges is None) else ranges[batch_idx, 0]
        offset_padded = (
            0
            if const_expr(ranges is None)
    ...
    def offset_batch(
        ...
        if const_expr(not self.has_ranges):
...
class SeqlenInfoQK:
    ...
    num_n_blocks: Int32
    has_ranges_q: cutlass.Constexpr[bool]
    has_ranges_k: cutlass.Constexpr[bool]
    ...
    def create(
        ...
        offset_q = 0 if const_expr(mRangesQ is None) else mRangesQ[batch_idx, 0]
        offset_k = 0 if const_expr(mRangesK is None) else mRangesK[batch_idx, 0]
        padded_offset_q = (
            0
        ...
        if const_expr(mSeqUsedQ is not None):
            ...
            seqlen_q = (
                seqlen_q_static
                if const_expr(mRangesQ is None)
                else mRangesQ[batch_idx, 1] - offset_q
            )
        if const_expr(mSeqUsedK is not None):
            ...
            seqlen_k = (
                seqlen_k_static
                if const_expr(mRangesK is None)
                else mRangesK[batch_idx, 1] - offset_k
            )
        m_block_offset = (
        ...
        return SeqlenInfoQK(
            ...
            block_idx_offset,
            num_n_blocks,
            has_ranges_q=mRangesQ is not None,
            has_ranges_k=mRangesK is not None,
    ...
    def offset_batch_Q(
        ...
        if const_expr(not ragged):
            if const_expr(not self.has_ranges_q):
    ...
    def offset_batch_K(
        ...
        if const_expr(not ragged):
            if const_expr(not self.has_ranges_k):
...
class SeqlenInfoQKNewK:
    ...
    def create(
        ...
        mRangesQ: Optional[cute.Tensor] = None,
        mRangesK: Optional[cute.Tensor] = None,
        mRangesKNew: Optional[cute.Tensor] = None,
        mSeqUsedQ: Optional[cute.Tensor] = None,
        mSeqUsedK: Optional[cute.Tensor] = None,
        ...
        if const_expr(mRangesK is not None):
            offset_k = mRangesK[batch_idx, 0] + leftpad_k
        else:
            offset_k = leftpad_k if const_expr(mRangesQ is not None) else 0
        offset_k_new = (
            0 if const_expr(mRangesKNew is None) else mRangesKNew[batch_idx, 0]
        )
        # seqlen_q
        if const_expr(mSeqUsedQ is not None):
            seqlen_q = mSeqUsedQ[batch_idx]
        elif const_expr(mRangesQ is not None):
            seqlen_q = mRangesQ[batch_idx, 1] - mRangesQ[batch_idx, 0]
        else:
        ...
        # seqlen_k_new
        if const_expr(mRangesKNew is None):
            seqlen_k_new = 0 if const_expr(mRangesQ is None) else shape_K_new_0
        else:
