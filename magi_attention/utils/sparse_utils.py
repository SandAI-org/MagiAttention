...
# ────────────────────────────────────────────────────────────────
# Direct sparse_kv_indices → kernel metadata (new direct path)
# ────────────────────────────────────────────────────────────────


def prepare_sparse_kv_metadata(
    sparse_kv_indices: torch.Tensor,
    actual_topk: list[int] | torch.Tensor | None,
    tile_size: int,
    ...
    """Build metadata for the sparse_kv_indices direct kernel path.

    The kernel reads ``sparse_kv_indices`` directly (local KV token IDs)
    and adds ``sparse_kv_batch_offsets[bidb]`` on the fly to get global
    row IDs.  No ``flat_token_ids``, ``q_ranges``, or ``k_ranges`` are needed.

    Args:
        sparse_kv_indices: ``[B, NHK, S, max_topk]`` int32, padding=-1.
        actual_topk: ``[B]`` – valid entries per batch in the last dim.
            If ``None``, every entry is treated as valid (no -1 padding).
        tile_size: Tile alignment for kBlockN (typically 128).
        kv_seqlens: ``[B]`` – per-batch KV sequence lengths for computing
            batch offsets.  If ``None``, all batches assumed to have the same
            KV length ``S`` (the 3rd dim of sparse_kv_indices).

    Returns:
        ``(loop_count, invalid_count, sparse_kv_batch_offsets, sparse_kv_indices_2d)``
    """
    B, NHK, S, max_topk = sparse_kv_indices.shape
    device = sparse_kv_indices.device

    if actual_topk is not None:
        if isinstance(actual_topk, torch.Tensor):
            atopk = actual_topk.tolist()
        else:
            atopk = list(actual_topk)
    else:
        atopk = [max_topk] * B

    num_unique_q = B * NHK * S

    # ── loop_count / invalid_count (pure arithmetic, per-batch) ──
    loop_list: list[int] = []
    invalid_list: list[int] = []
    for b in range(B):
        vk = atopk[b]
        lc = (vk + tile_size - 1) // tile_size
        aligned = lc * tile_size
        inv = aligned - vk
        loop_list.extend([lc] * (NHK * S))
        invalid_list.extend([inv] * (NHK * S))

    loop_count = torch.tensor(loop_list, dtype=torch.int32, device=device)
    invalid_count = torch.tensor(invalid_list, dtype=torch.uint8, device=device)

    # ── sparse_kv_batch_offsets: per-batch row offset into the K tensor ──
    # K tensor layout is (total_k, NHK, D) where total_k = sum of per-batch (NHK * S_b).
    # For batch b, the first K row is at offset = sum_{i<b} (NHK * S_i).
    if kv_seqlens is not None:
        if isinstance(kv_seqlens, torch.Tensor):
            kv_lens = kv_seqlens.tolist()
        else:
            kv_lens = list(kv_seqlens)
    else:
        kv_lens = [S] * B

    # Expand to [num_unique_q]: each unique Q row at index (b, h, q) gets the
    # global K row offset = sum_{i<b}(NHK * kv_lens[i]) + h * kv_lens[b].
    # The kernel adds this offset to the local token ID from sparse_kv_indices
    # to obtain the global K tensor row index.
    offsets_expanded: list[int] = []
    running = 0
    for b in range(B):
        for h in range(NHK):
    ...
    if max_aligned > max_topk:
        pad_width = max_aligned - max_topk
        last_col = indices_2d[:, -1:].expand(-1, pad_width)
        indices_2d = torch.cat([indices_2d, last_col], dim=1).contiguous()
