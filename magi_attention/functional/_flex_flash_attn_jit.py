...
def get_ffa_uri(
    ...
    return (
        ...
        f"{'_sparse_kv' if sparse_kv else ''}"
...
def sanity_check(
    ...
    if sparse_kv:
        assert direction == "fwd", "sparse_kv only take effect when direction == 'fwd'"
    if swap_bwd_qk_loop:
        assert (
            direction == "bwd"
...
def get_ffa_jit_spec(
    ...
    logger.info(
        ...
        "sparse_load=%s, sparse_kv=%s, profile_mode=%s, return_max_logits=%s",
        arch_sm_num,
        direction,
        head_dim,
        compute_dtype,
