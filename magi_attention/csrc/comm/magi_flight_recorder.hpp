...
//    auto-registered).
class TORCH_API MagiDebugInfoWriter {
 public:
  virtual ~MagiDebugInfoWriter() = default;
  virtual void write(const std::string& trace);
  static MagiDebugInfoWriter& getWriter(int rank);
  static void registerWriter(std::unique_ptr<MagiDebugInfoWriter> writer);
  virtual std::string getWriterTarget() {
    return filename_;
  }

 protected:
  MagiDebugInfoWriter(const std::string& namePrefix, int rank) {
    filename_ = c10::str(namePrefix, rank);
  }
  std::string filename_;

 private:
  static std::unique_ptr<MagiDebugInfoWriter> writer_;
