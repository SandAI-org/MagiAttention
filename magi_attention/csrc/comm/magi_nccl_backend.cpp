...
std::atomic<bool> MagiNCCLBackend::shouldDump_(false);
...
const int64_t MagiNCCLBackend::kWatchdogThreadSleepMillis = 100;
constexpr int64_t kSynchronizeBusyWaitMillis = 1;
thread_local uint64_t MagiNCCLBackend::ncclActiveGroupCounter_ = 0;

std::ostream& operator<<(
    std::ostream& output,
    const MagiNCCLBackend::WorkNCCL& workNCCL) {
  std::string workInfo;
  workInfo = c10::str(
      "WorkNCCL(",
      "SeqNum=",
...
MagiNCCLBackend::WorkNCCL::WorkNCCL(
    std::string pgUID,
    std::string pgDesc,
    at::Device& device,
    int rank,
  ...
  if (cudaEventCacheEnabled) {
    ncclStartEvent_ = enableTiming
        ? MagiNCCLBackend::CUDAEventCache::get(device.index())
              .create(enableTiming)
        : nullptr;
    ncclEndEvent_ = MagiNCCLBackend::CUDAEventCache::get(device.index())
                        .create(enableTiming);
  } else {
    ncclStartEvent_ = enableTiming
...
MagiNCCLBackend::WorkNCCL::WorkNCCL(const WorkNCCL& w)
...
MagiNCCLBackend::WorkNCCL::~WorkNCCL() = default;

bool MagiNCCLBackend::WorkNCCL::isCompleted() {
...
bool MagiNCCLBackend::WorkNCCL::isStarted() {
  if (!ncclComm_->isAborted()) {
    checkAndSetException();
  }
...
bool MagiNCCLBackend::WorkNCCL::isSuccess() const {
  C10_THROW_ERROR(NotImplementedError, "WorkNCCL::isSuccess() is deprecated");
}

void MagiNCCLBackend::WorkNCCL::checkAndSetException() {
  if (exception()) {
...
const std::string& MagiNCCLBackend::WorkNCCL::logPrefix() const {
  static std::string prefix = c10::str("[Rank ", rank_, "] ");
  return prefix;
}

void MagiNCCLBackend::WorkNCCL::setException(
...
bool MagiNCCLBackend::WorkNCCL::finishedGPUExecution() {
  checkAndSetException();
  return finishedGPUExecutionInternal();
}

bool MagiNCCLBackend::WorkNCCL::startedGPUExecutionInternal() const {
...
bool MagiNCCLBackend::WorkNCCL::finishedGPUExecutionInternal() const {
  // Checking the work's corresponding CUDA event's status
...
bool MagiNCCLBackend::WorkNCCL::checkTimeout(
    std::optional<std::chrono::milliseconds> timeout) {
  STATIC_SCOPED_WAIT_COUNTER(
      pytorch.wait_counter.MagiNCCLBackend__checkTimeout);
...
void MagiNCCLBackend::WorkNCCL::handleException(
  ...
  if (exception_) {
    ...
    C10_LOG_API_USAGE_ONCE("MagiNCCLBackend.WorkNCCL.handleException");
...
void MagiNCCLBackend::WorkNCCL::synchronize() {
  synchronizeStream();
  if (c10d::allow_inflight_collective_as_graph_input()) {
    c10d::unregister_work(
        c10::intrusive_ptr<
            MagiNCCLBackend::WorkNCCL>::unsafe_reclaim_from_nonowning(this));
  }
}

void MagiNCCLBackend::WorkNCCL::synchronizeStream() {
...
bool MagiNCCLBackend::WorkNCCL::wait(std::chrono::milliseconds timeout) {
  RECORD_PARAM_COMMS(
...
void MagiNCCLBackend::WorkNCCL::abort() {
...
MagiNCCLBackend::CUDAEventCache::CUDAEventCache() = default;
...
std::shared_ptr<at::cuda::CUDAEvent> MagiNCCLBackend::CUDAEventCache::create(
    bool timing) {
...
MagiNCCLBackend::CUDAEventCache& MagiNCCLBackend::CUDAEventCache::get(
  ...
  static thread_local std::
      map<at::DeviceIndex, MagiNCCLBackend::CUDAEventCache>
          cacheDeviceMap;
...
constexpr const char* MULTI_DEVICE_ERROR_MSG =
    ...
    "MagiNCCLBackend continues supporting multi-process and multi-thread modes.";

MagiNCCLBackend::MagiNCCLBackend(
  ...
  TORCH_CHECK_WITH(
      ...
      "MagiNCCLBackend is only supported with GPUs, no GPUs found!");
  ...
  // across different MagiNCCLBackend(PG) instances. We need to get the
  ...
  if (!blockingWait_) {
    ncclCommWatchdogThread_ =
        std::thread(&MagiNCCLBackend::ncclCommWatchdog, this);
  }
#endif
  ...
  LOG(INFO) << logPrefix() << "MagiNCCLBackend initialization options: "
  ...
  LOG(INFO) << logPrefix() << "MagiNCCLBackend environments: "
...
void MagiNCCLBackend::eagerConnectSingleDevice(at::Device device) {
...
bool MagiNCCLBackend::useNonblocking() {
...
void MagiNCCLBackend::performNocolorSplit(at::Device device) {
...
bool MagiNCCLBackend::isInitialized() {
...
void MagiNCCLBackend::registerMemPool(c10::cuda::MemPool* pool) {
...
void MagiNCCLBackend::deregisterMemPool(c10::cuda::MemPool* pool) {
  const auto key = std::to_string(pool->device());
  auto device = at::Device(at::DeviceType::CUDA, pool->device());
  LOG(INFO) << logPrefix()
...
c10::intrusive_ptr<intra_node_comm::IntraNodeComm> MagiNCCLBackend::
    initIntraNodeComm() {
...
void MagiNCCLBackend::setSequenceNumberForGroup() {
} // NCCL just starts sequence numbers at 0.

uint64_t MagiNCCLBackend::getSequenceNumberForGroup() {
  return seqCollective_;
}

void MagiNCCLBackend::registerOnCompletionHook(
  ...
  TORCH_CHECK_WITH(
      ...
      "MagiNCCLBackend OnCompletion hook already registered");

  TORCH_CHECK_WITH(
      ValueError,
      enableTiming_.load(),
      "MagiNCCLBackend OnCompletion hook requires recording start and end "
  ...
  onCompletionHookThread_ = std::thread(&MagiNCCLBackend::runHookLoop, this);
}

// must release GIL when calling this method
void MagiNCCLBackend::waitForPendingWorks() {
...
void MagiNCCLBackend::enableCollectivesTiming() {
  enableTiming_.store(true);
}

bool MagiNCCLBackend::waitForFutureOrTimeout(
    std::future<bool>& fut,
...
void MagiNCCLBackend::abortCommsFromMap(
  ...
  for (auto& it : ncclCommsMap) {
    ...
    VLOG(2) << logPrefix() << "MagiNCCLBackend destroying ncclComm_ "
    ...
    VLOG(2) << logPrefix() << "MagiNCCLBackend destroyed "
...
bool MagiNCCLBackend::abortComms(
    const std::optional<std::string>& abortReason) {
...
// Abort this backend.
void MagiNCCLBackend::abort() {
  // This will log counter for how long the abort actually takes.
  STATIC_SCOPED_WAIT_COUNTER(pytorch.MagiNCCLBackend__abort);
  ...
  LOG(INFO) << logPrefix()
            << "Launching MagiNCCLBackend abort asynchrounously.";
  std::future<bool> fut =
  ...
  LOG(INFO) << logPrefix() << "MagiNCCLBackend aborts successfully.";
...
// NOLINTNEXTLINE(bugprone-exception-escape)
MagiNCCLBackend::~MagiNCCLBackend() {
  LOG(INFO) << logPrefix() << "MagiNCCLBackend destructor entered.";
...
join_threads:
  ...
  if (ncclCommWatchdogThread_.joinable()) {
    ncclCommWatchdogThread_.join();
    LOG(INFO) << logPrefix() << "MagiNCCLBackend watchdog thread joined.";
  }
  if (ncclHeartbeatMonitorThread_.joinable()) {
    ncclHeartbeatMonitorThread_.join();
    LOG(INFO) << logPrefix()
              << "MagiNCCLBackend heart beat monitor thread joined.";
  }
  if (onCompletionHookThread_.joinable()) {
    onCompletionHookThread_.join();
    LOG(INFO) << logPrefix()
              << "MagiNCCLBackend onCompletionHookThread thread joined.";
  }
}

bool MagiNCCLBackend::dumpDebuggingInfo(bool includeStackTrace /*=true*/) {
  ...
  LOG(ERROR)
      << logPrefix()
      << "MagiNCCLBackend preparing to dump debug info. Include stack trace: "
      << includeStackTrace;
...
void MagiNCCLBackend::terminateProcess(const std::string& errMsg) {
...
std::string MagiNCCLBackend::getNCCLWatchdogTimeoutErrorMsg(
...
std::string MagiNCCLBackend::getNCCLWatchdogTimeoutExitMsg(
...
void MagiNCCLBackend::heartbeatMonitor() {
  ...
  while (true) {
    ...
    if (computeDeltaMS(lastTimeHeartBeatCheck, currentTime) >=
      ...
      if (heartbeat != heartBeatCounter) {
        ...
        errorMsg = c10::str(
            logPrefix(),
            "MagiNCCLBackend's watchdog got stuck for ",
            heartbeatTimeoutInSec_,
        ...
        exitReason = "MagiNCCLBackend watchdog hang";
        break;
      }
    }
  ...
  if (!terminateHeartbeatMonitorThread_.load()) {
    ...
    if (monitorThreadEnabled_.load()) {
      ...
      LOG(ERROR)
          << logPrefix()
          << "MagiNCCLBackend monitor thread is disabled, but would have terminated the process"
...
void MagiNCCLBackend::DesyncDebugger::init(
    int rank,
    int size,
    c10::intrusive_ptr<Store> store) {
  rank_ = rank;
...
void MagiNCCLBackend::DesyncDebugger::run() {
  if (!enabled_)
    return;
...
void MagiNCCLBackend::DesyncDebugger::logWorkStart(WorkNCCL& work) {
  if (!enabled_)
    return;
...
void MagiNCCLBackend::DesyncDebugger::logWorkEnd(WorkNCCL& work) {
  if (!enabled_)
    return;
...
// prefix. PG ID records how many MagiNCCLBackend objects were created on a
...
std::string MagiNCCLBackend::createLogPrefix() const {
...
const std::string& MagiNCCLBackend::logPrefix() const {
  return logPrefix_;
}

const int& MagiNCCLBackend::globalRank() const {
  static int globalRank = rank_;
  return globalRank;
}

const std::vector<uint64_t>& MagiNCCLBackend::groupRanks() const {
...
void MagiNCCLBackend::broadcastDumpSignal() {
  try {
...
void MagiNCCLBackend::watchdogHandler() {
  bool done = false;
  lastWorkListUpdateTime_ = std::chrono::steady_clock::now();
  auto lastStatusUpdateTime = std::chrono::steady_clock::now();
  std::list<MagiNCCLBackend::WorkNCCL> completedWorkList;
...
void MagiNCCLBackend::runHookLoop() {
    ...
    try {
      ...
      if (std::string(e.what()).find("driver shutting down") !=
        ...
        const auto errorStr = c10::str(
            ...
            rank_,
            " while running onCompletion hook for MagiNCCLBackend: ",
            e.what(),
        ...
        // the process Abort all NCCL Communicators on this MagiNCCLBackend
        // instance.
        abortComms(errorStr);
      }
    }
...
std::exception_ptr MagiNCCLBackend::WorkNCCL::checkForNCCLErrors() {
...
std::exception_ptr MagiNCCLBackend::checkForNCCLErrorsInternal(
  ...
  if (commFailureReason != std::nullopt) {
    return std::make_exception_ptr(C10_BUILD_ERROR(
        DistBackendError,
        c10::str(
            "NCCL communicator encountered error set by MagiNCCLBackend: ",
            *commFailureReason)));
  }
...
void MagiNCCLBackend::broadcastUniqueNCCLID(
    ncclUniqueId* ncclID,
    bool isSingleP2POp,
...
void MagiNCCLBackend::destroyNCCLComms(const std::string& devNCCLCommMapKey) {
  std::lock_guard<std::mutex> lock(mutex_);
...
std::shared_ptr<MagiNCCLComm> MagiNCCLBackend::initNCCLComm(
  ...
  if (!ncclComm) {
    if (getCvarBool(TORCH_NCCL_BCAST_UNIQUEID, true) && !isSendRecvSelf) {
      ...
      auto timerDeltaMs =
          ...
          1000;
      LOG(INFO) << logPrefix()
                << "MagiNCCLBackend broadcast unique ID through store took "
                << timerDeltaMs << " ms";
    }
  ...
  VLOG(2) << logPrefix() << "MagiNCCLBackend created ncclComm_ "
          << ncclComm->repr()
...
uint64_t MagiNCCLBackend::getCommSplitCounter() const {
  uint64_t ret = 0;
...
c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL> MagiNCCLBackend::initWork(
    at::Device& device,
    int rank,
    OpType opType,
    bool isP2P,
    ...
    bool record) {
  auto r = c10::make_intrusive<MagiNCCLBackend::WorkNCCL>(
      pg_uid_,
      pg_desc_,
      device,
      rank,
...
std::vector<at::Tensor> MagiNCCLBackend::WorkNCCL::result() {
  return *outputs_;
}

c10::intrusive_ptr<c10::ivalue::Future> MagiNCCLBackend::WorkNCCL::
    getFuture() {
  return future_;
}
...
float MagiNCCLBackend::WorkNCCL::getDuration() const {
...
uint64_t MagiNCCLBackend::WorkNCCL::getSequencenumber() const {
  return seq_;
}

void MagiNCCLBackend::assignTimeoutToWork(
    const c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL>& work,
    const c10::intrusive_ptr<MagiNCCLBackend::Options>& option) {
...
void MagiNCCLBackend::workEnqueue(
    const c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL>& work) {
...
void MagiNCCLBackend::startCoalescing() {
...
// REDUCE_SCATTER
c10::intrusive_ptr<Work> MagiNCCLBackend::endCoalescing(OpType optype) {
...
c10::intrusive_ptr<Work> MagiNCCLBackend::endCoalescing() {
  // Default OpType to COALESCED if not specified
  return endCoalescing(OpType::COALESCED);
}

template <typename Fn, typename PreProcess, typename PostProcess>
c10::intrusive_ptr<Work> MagiNCCLBackend::collective(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::collectiveCoalesced(
    std::vector<at::Tensor>& inputs,
    std::vector<at::Tensor>& outputs,
    Fn fn,
    OpType opType,
...
template <typename Fn, typename PreProcess, typename PostProcess>
c10::intrusive_ptr<Work> MagiNCCLBackend::pointToPoint(
    at::Tensor& tensor,
    Fn fn,
    int peer,
    OpType opType,
  ...
  // to wait() on the returned handle, so MagiNCCLBackend can't know
  ...
  c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL> work;
  if (coalescing_state_) {
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allreduce_sparse(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allreduce_impl(
    at::Tensor& tensor,
    const char* profilingTitle,
    const AllreduceOptions& opts) {
  return collective(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allreduce(
    std::vector<at::Tensor>& tensors,
    const AllreduceOptions& opts) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
  auto tensor = tensors.back();
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allreduce_coalesced(
    std::vector<at::Tensor>& tensors,
...
c10::intrusive_ptr<Work> MagiNCCLBackend::broadcast(
    std::vector<at::Tensor>& tensors,
...
c10::intrusive_ptr<Work> MagiNCCLBackend::_broadcast_oop(
    at::Tensor& outputTensor,
...
c10::intrusive_ptr<Work> MagiNCCLBackend::reduce(
    std::vector<at::Tensor>& tensors,
...
c10::intrusive_ptr<Work> MagiNCCLBackend::_reduce_oop(
    at::Tensor& outputTensor,
    at::Tensor& inputTensor,
    const ReduceOptions& opts) {
  if (outputTensor.numel() != inputTensor.numel()) {
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allgather(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::allgather_coalesced(
    std::vector<std::vector<at::Tensor>>& /* unused */,
    std::vector<at::Tensor>& /* unused */,
    const AllgatherOptions& /* unused */) {
  C10_THROW_ERROR(
      NotImplementedError,
      "MagiNCCLBackend does not support allgather_coalesced");
}

c10::intrusive_ptr<Work> MagiNCCLBackend::allgather_into_tensor_coalesced(
    std::vector<at::Tensor>& outputs,
    std::vector<at::Tensor>& inputs,
    const AllgatherOptions& opts) {
  // @lint-ignore CLANGTIDY
...
c10::intrusive_ptr<Work> MagiNCCLBackend::reduce_scatter(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::_reduce_scatter_base(
    at::Tensor& outputTensor,
    at::Tensor& inputTensor,
    const ReduceScatterOptions& opts) {
...
c10::intrusive_ptr<Work> MagiNCCLBackend::reduce_scatter_tensor_coalesced(
    std::vector<at::Tensor>& outputs,
    std::vector<at::Tensor>& inputs,
    const ReduceScatterOptions& opts) {
  TORCH_CHECK(
...
c10::intrusive_ptr<Work> MagiNCCLBackend::barrier(const BarrierOptions& opts) {
  RECORD_PARAM_COMMS(
      std::make_tuple(
          static_cast<int64_t>(seqCollective_) + 1,
          false), // seq + 1 to match collective
  ...
  auto ncclWork = dynamic_cast<MagiNCCLBackend::WorkNCCL*>(work.get());
  TORCH_CHECK(ncclWork);
  ncclWork->isBarrierOp_ = true;
  return work;
}

c10::intrusive_ptr<Work> MagiNCCLBackend::alltoall_base(
    at::Tensor& outputTensor,
    at::Tensor& inputTensor,
...
c10::intrusive_ptr<Work> MagiNCCLBackend::alltoall(
    std::vector<at::Tensor>& outputTensors,
    std::vector<at::Tensor>& inputTensors,
    const AllToAllOptions& /* unused */) {
  std::vector<int64_t> inSplitSizes;
...
c10::intrusive_ptr<Work> MagiNCCLBackend::send(
    std::vector<at::Tensor>& tensors,
    int dstRank,
    int /* unused */) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
...
c10::intrusive_ptr<Work> MagiNCCLBackend::recv(
    std::vector<at::Tensor>& tensors,
    int srcRank,
    int /* unused */) {
  TORCH_CHECK(tensors.size() == 1, MULTI_DEVICE_ERROR_MSG);
...
void MagiNCCLBackend::groupStart() {
  C10D_NCCL_CHECK(ncclGroupStart(), std::nullopt);
  ++ncclActiveGroupCounter_;
}

void MagiNCCLBackend::groupEnd() {
  C10D_NCCL_CHECK(ncclGroupEnd(), std::nullopt);
  --ncclActiveGroupCounter_;
}
...
c10::intrusive_ptr<Work> MagiNCCLBackend::gather(
    std::vector<std::vector<at::Tensor>>& outputTensors,
    std::vector<at::Tensor>& inputTensors,
    const GatherOptions& opts) {
  static auto invalidArgument = [](const std::string& msg) {
    C10_THROW_ERROR(ValueError, "MagiNCCLBackend::gather: " + msg);
  };
...
c10::intrusive_ptr<Work> MagiNCCLBackend::scatter(
    std::vector<at::Tensor>& outputTensors,
    std::vector<std::vector<at::Tensor>>& inputTensors,
    const ScatterOptions& opts) {
  static auto invalidArgument = [](const std::string& msg) {
    C10_THROW_ERROR(ValueError, "MagiNCCLBackend::scatter: " + msg);
  };
...
c10::intrusive_ptr<Work> MagiNCCLBackend::recvAnysource(
    std::vector<at::Tensor>& /* unused */,
    int /* unused */) {
  C10_THROW_ERROR(
      NotImplementedError, "MagiNCCLBackend does not support recvAnysource");
}

c10::intrusive_ptr<Work> MagiNCCLBackend::_allgather_base(
