...
void MagiDebugInfoWriter::write(const std::string& trace) {
...
MagiDebugInfoWriter& MagiDebugInfoWriter::getWriter(int rank) {
  if (writer_ == nullptr) {
    ...
    std::unique_ptr<MagiDebugInfoWriter> writerPtr(
        new MagiDebugInfoWriter(fileNamePrefix, rank));
    MagiDebugInfoWriter::registerWriter(std::move(writerPtr));
  }
  return *writer_;
}

void MagiDebugInfoWriter::registerWriter(std::unique_ptr<MagiDebugInfoWriter> writer) {
  TORCH_CHECK_WITH(
      DistBackendError,
...
std::string MagiFlightRecorder::Entry::getTraceback() {
...
std::optional<size_t> MagiFlightRecorder::record(
    size_t pg_id,
...
void MagiFlightRecorder::record_pg_ranks(
    const std::tuple<std::string, std::string>& pg_name,
...
void MagiFlightRecorder::update_state(Entry& r) {
  if (r.start_ != nullptr) {
...
std::vector<MagiFlightRecorder::Entry> MagiFlightRecorder::dump_entries() {
  std::lock_guard<std::mutex> guard(mutex_);
...
// std::nullopt.
std::optional<MagiFlightRecorder::Entry> MagiFlightRecorder::getEntry(
...
void MagiFlightRecorder::retire_id(
    std::optional<size_t> id,
...
const c10::List<c10::IValue> MagiFlightRecorder::getCollectiveTrace(
...
const c10::Dict<c10::IValue, c10::IValue> MagiFlightRecorder::getPgConfig() {
...
const std::map<std::string, std::map<std::string, std::string>> MagiFlightRecorder::
    getPgConfigJson() {
...
const c10::Dict<c10::IValue, c10::IValue> MagiFlightRecorder::getPgStatus() {
...
std::string MagiFlightRecorder::dump(
    ...
    bool includeStackTraces,
    bool onlyActive) {
  STATIC_SCOPED_WAIT_COUNTER(pytorch.wait_counter.MagiFlightRecorder__dump);
...
std::unique_ptr<MagiDebugInfoWriter> MagiDebugInfoWriter::writer_ = nullptr;
std::atomic<bool> MagiDebugInfoWriter::hasWriterRegistered_(false);
