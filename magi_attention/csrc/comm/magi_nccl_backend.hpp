...
// Enable recording start-events for all MagiNCCLBackend collectives, and
...
// Enable monitoring thread which aborts the process when the MagiNCCLBackend
...
constexpr auto kMagiNCCLBackendDefaultTimeout =
...
// If set, MagiNCCLBackend doesn't use recordStream calls to ensure
...
// If set, MagiNCCLBackend registers postAlloc and preFree hooks to cuda cache
// allocator so that whenever a tensor is allocated or freed, MagiNCCLBackend
...
// MagiNCCLBackend implements NCCL bindings for c10d.
//
...
// provided by MagiNCCLBackend to check if the NCCL operation of WorkNCCL has
// finished execution on the GPU (not just scheduled).
//
// Example on using the NCCL process group
//
//   MagiNCCLBackend pg(store, rank, size);
...
class TORCH_API MagiNCCLBackend : public Backend {
 public:
  class WorkNCCL : public Work, public std::enable_shared_from_this<WorkNCCL> {
   ...
   protected:
    ...
    // Clone of blockingWait_ from MagiNCCLBackend.
    bool blockingWait_{false};

    // Clone of avoidRecordStreams_ from MagiNCCLBackend.
    bool avoidRecordStreams_{false};

    // Clone of opTimeout_ from MagiNCCLBackend.
  ...
  struct Options : Backend::Options {
    // NOTE: timeout in MagiNCCLBackend::Options denote the timeout for
    ...
    // via `ncclCommSplit`
    std::shared_ptr<MagiNCCLBackend> split_from;
  ...
  C10_DEPRECATED MagiNCCLBackend(
  ...
  // instead of relying on MagiNCCLBackend destructor.
  void abort();
 ...
 protected:
  ...
  // workEnqueue
  virtual c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL> initWork(
      at::Device& device,
      int rank,
      OpType opType,
      bool isP2P,
 ...
 private:
  ...
  // cache. This can be done cleanly by having a thread for the MagiNCCLBackend
  // class. Attempting to modify the communicator cache from the WorkNCCL class
  // might run into issues with object lifetime since the MagiNCCLBackend
  ...
  // devices. It is called in the constructor of MagiNCCLBackend, so it always
 ...
 protected:
  ...
  // Interval of check coordinated signals in MagiNCCLBackend from other ranks
  ...
  std::list<MagiNCCLBackend::WorkNCCL> workMetaList_;
  ...
  std::list<MagiNCCLBackend::WorkNCCL> completedWorkList_;

  // Add Work Pointer to workVector
  void workEnqueue(const c10::intrusive_ptr<MagiNCCLBackend::WorkNCCL>&);
  ...
  // The number of MagiNCCLBackend created on the current rank.
  size_t local_id_;
