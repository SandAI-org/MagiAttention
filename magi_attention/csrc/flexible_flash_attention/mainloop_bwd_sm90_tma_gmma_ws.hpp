...
struct CollectiveMainloopBwdSm90 {
  ...
  // When enabling sparse load, 2 warps for sparse load and 2 warps for dK/dV store
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    ...
    return {
        ...
        /*sparse_load_invalid+count=*/args.sparse_load_invalid_count,
...
#pragma unroll(kHeadDim < 256 ? 2 : 1)
  ...
  CUTLASS_DEVICE bool load_with_loop_k(
    ...
    auto load_K = [&, mcast_mask_kv = mcast_mask_kv, cluster_block_id_kv = cluster_block_id_kv](int const n_block_idx, int offset_k) {
      ...
      Tensor mK = params.tma_load_K.get_tma_tensor(params.shape_KVdKdV)(_, _, block_meta.bidh_kv); // (seqlen_kv, head_dim)
    ...
    auto load_V = [&, mcast_mask_kv = mcast_mask_kv, cluster_block_id_kv = cluster_block_id_kv](int const n_block_idx, int offset_k) {
      ...
      Tensor mV = params.tma_load_V.get_tma_tensor(params.shape_KVdKdV)(_, _, block_meta.bidh_kv); // (seqlen_kv, head_dim)
    ...
    auto load_QdO_LSE_dPsum = [&]() {
      Tensor mQ = params.tma_load_Q.get_tma_tensor(params.shape_QdOdQ)(_, _, block_meta.bidh); // (seqlen_q, head_dim)
      Tensor mdO = params.tma_load_dO.get_tma_tensor(params.shape_QdOdQ)(_, _, block_meta.bidh); // (seqlen_q, head_dim)
      Tensor mLSE = make_tensor(make_gmem_ptr(params.ptr_LSE_log2), params.shape_LSEdPsum, params.stride_LSE)(_, _, block_meta.bidh); // (4, seqlen_q)
      Tensor mdPsum = make_tensor(make_gmem_ptr(params.ptr_dPsum), params.shape_LSEdPsum, params.stride_dPsum)(_, _, block_meta.bidh); // (4, seqlen_q)

      Tensor gQ = local_tile(domain_offset(make_coord(offset_q, _0{}), mQ), select<0, 2>(TileShape_MNK{}), make_coord(block_meta.m_block, _0{})); // (M, K)
      Tensor gdO = local_tile(domain_offset(make_coord(offset_q, _0{}), mdO), select<0, 2>(TileShape_MNK{}), make_coord(block_meta.m_block, _0{})); // (M, K)
      Tensor gLSE = local_tile(cute::domain_offset(make_coord(_0{}, offset_q), mLSE), make_shape(_4{}, Int<kBlockM>{}), make_coord(_0{}, block_meta.m_block)); // (4, M)
      Tensor gdPsum =
          local_tile(cute::domain_offset(make_coord(_0{}, offset_q), mdPsum), make_shape(_4{}, Int<kBlockM>{}), make_coord(_0{}, block_meta.m_block)); // (4, M)
...
#pragma unroll 2
  ...
  // q for outer-loop and k for inner-loop
  template <typename SharedStorage, typename BlockMetaT>
  CUTLASS_DEVICE void store_dkv(Params const& params, SharedStorage& shared_storage, cute::tuple<int32_t, int32_t, int32_t> block_coord, BlockMetaT& block_meta) {
    ...
    // SparseLoad scatter-store path: early return
    if constexpr (SparseLoad) {
...
#pragma unroll
    ...
    auto store_dv_this_n_block = [&](int const n_block_idx, int offset_k) {
      Tensor sdV = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_dvacc.data()), SmemLayoutdKVaccumTMA{});
      Tensor mdVaccum = params.tma_add_dV.get_tma_tensor(params.shape_KVdKdV)(_, _, block_meta.bidh_kv); // (seqlen_kv, head_dim)
...
#pragma unroll
    ...
    auto store_dk_this_n_block = [&](int const n_block_idx, int offset_k) {
      Tensor sdK = make_tensor(make_smem_ptr(shared_storage.tensors.mainloop.smem_dkacc.data()), SmemLayoutdKVaccumTMA{});
      Tensor mdKaccum = params.tma_add_dK.get_tma_tensor(params.shape_KVdKdV)(_, _, block_meta.bidh_kv); // (seqlen_kv, head_dim)
