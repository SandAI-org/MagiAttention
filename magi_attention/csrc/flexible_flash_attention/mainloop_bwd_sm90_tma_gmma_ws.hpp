...
struct CollectiveMainloopBwdSm90 {
  ...
  // InnerLoad_Tma && InnerUseScatter bypasses 1D bulk-reduce entirely (2D TMA reduce instead),
  ...
  struct TensorStorageLoopK : cute::aligned_struct<maxSmemAlignment> {
    ...
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutQ>, SmemAlignmentQKVdO> smem_q;
    cute::array_aligned<Element, cute::cosize_v<SmemLayoutdO>, SmemAlignmentQKVdO> smem_do;
    // dK and dV accumulators: when UnunionDkvacc, separate buffers allow dV/dK r2s
  ...
  static void prefetch_tma_descriptors(Params const& params) {
    ...
    if constexpr (!InnerUseScatter || InnerLoad_Tma) {
  ...
  CUTLASS_DEVICE bool load_with_loop_q(
    ...
    // InnerLoad_Tma && InnerUseScatter: absolute-coordinate TMA tensors (no domain_offset by offset_q)
  ...
  CUTLASS_DEVICE bool load_with_loop_k(
    ...
    // InnerLoad_Tma && InnerUseScatter (InnerLoopK scatter TMA): absolute-coordinate
...
#pragma unroll
  ...
  CUTLASS_DEVICE bool mma_with_loop_k(
    ...
    auto rebind_dKV_accum_tiles = [&]() {
      ...
      if constexpr (!dKVacc_use_smem || (DkvaccBypassSmem && !InnerUseScatter)) {
