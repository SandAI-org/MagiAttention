...
std::tuple<Flash_bwd_params, at::Tensor, at::Tensor, at::Tensor, at::Tensor> prepare_mha_bwd(
  ...
  at::Tensor outer_determin_range_locks = torch::empty({(total_k + kBlockN - 1) / kBlockN + 1, num_heads_kv * 2}, opts.dtype(torch::kInt32));
  ...
  at::Tensor inner_outer_determin_range_locks = torch::empty({(total_q_dq_det + kBlockM - 1) / kBlockM + 1, num_heads_dq_det * 2}, opts.dtype(torch::kInt32));
  // Initialize outer_determin_conflict_state, num_sm rows, ceil_div(total_k, kBlockN) + 1 columns
  ...
  // outer_determin_conflict_state a 0-element tensor (null data_ptr). The deterministic scheduler then
  ...
  at::Tensor outer_determin_conflict_state = torch::empty({num_sm, (total_k + kBlockN - 1) / kBlockN + 1}, opts.dtype(torch::kInt32));
  at::Tensor inner_outer_determin_conflict_state = torch::empty({num_sm, (total_q_dq_det + kBlockM - 1) / kBlockM + 1}, opts.dtype(torch::kInt32));

  // If deterministic is enabled, we need to zero out the out_accum tensor and conflict state
  if constexpr (Deterministic) {
    outer_determin_range_locks.zero_();
    outer_determin_conflict_state.zero_();
    inner_outer_determin_range_locks.zero_();
    inner_outer_determin_conflict_state.zero_();
  }
  ...
  set_params_dgrad(
      ...
      /*inner_outer_determin_conflict_state=*/inner_outer_determin_conflict_state.data_ptr(),
      /*inner_outer_determin_range_locks=*/inner_outer_determin_range_locks.data_ptr(),
