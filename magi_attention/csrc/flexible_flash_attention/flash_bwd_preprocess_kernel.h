...
#pragma unroll
      for (int mi = 0; mi < size(dP_sum); ++mi) {
        ...
        if (row < seqlen_o - m_block * kBlockM) {
          //if (dP_sum(mi) == -INFINITY) {
          //  printf("bidb: %d bidh: %d thread_idx: %d m_block: %d row: %d dp_sum: %f\n", bidb, bidh, thread_idx, m_block, row, dP_sum(mi));
          //}
          gdPsum(row) = dP_sum(mi);
          //if (gdPsum(row) == -INFINITY) {
          //  printf("bidb: %d bidh: %d thread_idx: %d m_block: %d row: %d dp_sum: %f gdPsum: %f\n", bidb, bidh, thread_idx, m_block, row, dP_sum(mi), gdPsum(row));
          //}
        }
        /*
        if (bidb != params.batch_size - 1) {
          if (row < seqlen_o - m_block * kBlockM)
            gdPsum(row) = dP_sum(mi);
        }
        else {
    ...
    if (thread_idx < seqlen_o - m_block * kBlockM && thread_idx < kBlockM) {
    ...
    if (bidb != params.batch_size - 1) {
      ...
      if(thread_idx < seqlen_rounded - m_block * kBlockM && thread_idx >= seqlen_o - m_block * kBlockM && bidh == 0) {
        printf("thread_idx: %d lse: %f seqlen_rounded: %d, kblockm: %d\n", thread_idx, lse, seqlen_rounded, kBlockM);
      }
    ...
    */
    ...
    /*
    if (bidb == 0 && bidh == 0 && thread_idx == 0) {
      printf("gLSElog2\n"); print_tensor(gLSElog2);
      printf("mLSElog2\n"); print_tensor(mLSElog2);
      printf("mdPsum\n"); print_tensor(mdPsum);
      printf("gdPsum"); print_tensor(gdPsum);
    }
    */
