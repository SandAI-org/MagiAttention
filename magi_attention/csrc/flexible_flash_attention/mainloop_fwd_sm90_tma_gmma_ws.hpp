...
#pragma unroll(Use_TMA_KV ? 2 : 1)
  ...
  CUTLASS_DEVICE bool mma(
    ...
    if constexpr (SparseLoad) {
      // SparseLoad: left-to-right traversal
      n_block = 0;
      n_block_min = 0;
      n_block_max = block_meta.loop_count;
    } else {
      // Dense and IndexAttn: right-to-left traversal
    ...
    auto boundary_mask_fn = [&](auto& tSrS, int n_block, auto const& attn_type, int const& seqlen_q, int const& seqlen_k) {
      if constexpr (!SparseLoad) {
        mask.template apply<true /*Seqlenk_mask*/, PackGQA, QheadPerKhead>(tSrS, block_meta.m_block, n_block, attn_type, thread_idx, seqlen_q, seqlen_k);
      } else {
...
#pragma unroll 2
    do {
      ...
      auto fwd_step = [&](int const n_block, auto mask_fn, auto check_inf_type) {
        ...
        // Apply mask: dense path uses the passed-in mask_fn; SparseLoad/IndexAttn skip (full attention only)
        if constexpr (!(SparseLoad || IndexAttn)) {
...
#pragma unroll 1
          ...
          // Separate masking iterations on the left for inv-causal and bi-causal attention
...
#pragma unroll 1
        ...
        // SparseLoad path: left-to-right traversal
        n_block = block_meta.cur_loop;
