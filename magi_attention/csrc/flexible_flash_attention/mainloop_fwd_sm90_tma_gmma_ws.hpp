...
struct CollectiveMainloopFwdSm90 {
    ...
    to_underlying_arguments(Arguments const& args) {
        ...
        return {args.ptr_Q, args.shape_Q, args.stride_Q, shape_Q_packed, stride_Q_packed,
                ...
                args.seqused_q, args.seqused_k, args.leftpad_k, args.seqlens_rotary, args.attn_type_map};
    }
    ...
    mma(Params const& params,
        ...
        if constexpr (IntraWGOverlap) {
            ...
            //     printf("============================================ tSrS m_block: %d ==============================\n", m_block);
            // }
            mask.template apply<true /*Seqlenk_mask*/, Is_causal, Is_local>(tSrS, m_block, n_block, attn_type);
            // if (bidb == 1 && bidh == 0 && thread_idx == 255 && m_block == 1) {
            ...
            if (attn_type == flash::AttnType::InvCausal || attn_type == flash::AttnType::BiCausal){
                ...
                // if (bidh == 0 && thread_idx == 0) {
                //     printf("bidb: %d, bidh: %d, m_block: %d, n_block: %d, n_block_min: %d\n", bidb, bidh, m_block, n_block, n_block_min);
                // }
            }
    ...
    load_kv_new(Params const& params,
        ...
        bool const is_causal = params.attn_type_map ? params.attn_type_map[bidb] : false;
        int const window_size_right = is_causal ? 0 : params.window_size_right;
        auto [n_block_new_min, n_block_new_max] = BlockMN_t::get_n_block_k_new_min_max(
            seqlen_info, m_block, bidb, split_idx, params.num_splits,
            params.window_size_left, window_size_right, params.qhead_per_khead_divmod, is_causal);
