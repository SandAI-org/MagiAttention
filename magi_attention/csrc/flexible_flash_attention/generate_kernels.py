...
template void run_mha_fwd_<{ARCH}, {DTYPE}, {DTYPE_OUT}, {HEAD_DIM}, {SOFTCAP}>(Flash_fwd_params &params, cudaStream_t stream);
#endif
"""
...
class Kernel:
    ...
    def filename(self) -> str:
        return f"flash_{self.direction}_hdim{self.head_dim}_{self.dtype}_{self.dtype_out}_sm{self.sm}.cu"
...
def batch_softcap(kernels_all) -> List[KERNEL_BATCH]:
    for dtype, head_dim, sm in itertools.product(
        ...
        if len(kernels) > 0:
            filename = f"flash_fwd_hdim{head_dim}_{dtype}_softcapall_sm{sm}.cu"
            template = "\n".join([f'#include "{k.filename}"' for k in kernels])
            yield KERNEL_BATCH(template, filename)
