...
std::tuple<Flash_fwd_params, at::Tensor, at::Tensor, std::optional<at::Tensor>> prepare_mha_fwd(
  ...
  // Initialize outer_determin_range_locks tensor, the shape is same as range_locks
  at::Tensor outer_determin_range_locks = torch::empty({(total_seqlen_q + kBlockM - 1) / kBlockM + 1, num_heads * 2}, opts.dtype(torch::kInt32));
  // Initialize outer_determin_conflict_state, num_sm rows, ceil_div(total_q, kBlockM) + 1 columns
  ...
  // now the shape of outer_determin_conflict_state is (num_sm, ceil_div(total_q, kBlockM) + 1, num_heads_kv)
  at::Tensor outer_determin_conflict_state = torch::empty({num_sm, (total_seqlen_q + kBlockM - 1) / kBlockM + 1, num_heads_kv}, opts.dtype(torch::kInt32));
  ...
  set_params_fprop(
      ...
      /*outer_determin_range_locks=*/Deterministic ? outer_determin_range_locks.data_ptr() : nullptr,
      /*outer_determin_conflict_state=*/Deterministic ? outer_determin_conflict_state.data_ptr() : nullptr,
