...
public:
    ...
    to_underlying_arguments(TileSchedulerArguments const& args) {
        ...
        return {args.num_head, args.num_batch,
                ...
                args.num_splits_dynamic_ptr, args.determin_conflict_state};
    }
        ...
        get_block_coord(Params const& params) const {
            if constexpr (!Split) {
                if constexpr (!Deterministic) {
                    ...
                    return {block, bidh, bidb, 0 /*split_idx*/, cute::get<0>(conflict_bidb), cute::get<1>(conflict_bidb)};
                }
            } else {
                ...
                if constexpr (!Deterministic) {
                    ...
                    return {block, bidh_actual, bidb, split_idx, cute::get<0>(conflict_bidb), cute::get<1>(conflict_bidb)};
                }
            }
        }
    };
        ...
        if constexpr (!Deterministic) {
            ...
            auto get_conflict_bidb = [&] (int bidb_last, int bidb_now, int block_now) {
                ...
                while (bidb_last < bidb_now) {
                    int l = params.q_ranges[2 * bidb_last] / kBlock + lane;
                    int r = (params.q_ranges[2 * bidb_last + 1] - 1) / kBlock;
                    while(l <= r) {
                ...
                int conflict_o_index1 = params.q_ranges[2 * bidb_now] / kBlock + block_now;
                int conflict_o_index2 = (params.q_ranges[2 * bidb_now] + kBlock - 1) / kBlock + block_now;
                __syncwarp();
