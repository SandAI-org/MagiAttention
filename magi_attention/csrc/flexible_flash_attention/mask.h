...
#pragma unroll
  ...
  CUTLASS_DEVICE void apply_q_mask(Tensor<Engine, Layout>& tSrS, const int m_block) const {
    ...
    if (m_block == 1 && thread_idx == 1) {
      ...
      printf("Inputs: seqlen_q = %d\n", seqlen_q);
      printf("Calculated: thread_row_offset = %d\n", thread_row_offset);
      printf("Calculated: seqlenq_row_limit = %d\n", seqlenq_row_limit);
      printf("----------------------------------------\n");
    }
    */
    ...
    for (int m = 0; m < cute::size<0>(tSrS_rowcol); ++m) {
      if (m >= seqlenq_row_limit) {
        /*
         ...
         printf("----------------------------------------\n");
        */
        for (int n = 0; n < cute::size<1>(tSrS_rowcol); ++n) {
          tSrS_rowcol(m, n) = -cutlass::platform::numeric_limits<float>::infinity();
        }
      }
    }
    // printf("apply_q_mask done!\n");
  }
};
