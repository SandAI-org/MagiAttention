...
class AttnRectangle {
public:
    AttnRectangle(const AttnRange& q,
                  const AttnRange& k,
                  const AttnRange& d,
    ...
    AttnRectangle(const AttnRectangle& rec)
        : q_range(rec.q_range),
          k_range(rec.k_range),
          d_range(rec.d_range) {
    ...
    bool is_valid() const {
        if (q_range.is_valid_open() && k_range.is_valid_open() && d_range.is_valid_close()) {
            return true;
        }
        return false;
    }
    ...
    void check_valid() const {
        if (!is_valid()) {
            throw std::invalid_argument(
                std::string("Some of the ")
                + "q_range=" + q_range.to_string() + " "
                + "k_range=" + k_range.to_string() + " "
                + "d_range=" + d_range.to_string()
    ...
    std::pair<
        std::optional<AttnRectangle>,
        std::optional<AttnRectangle>
    > cut_q(int cut_pos) const {
    ...
    bool is_full() const {
        return d_range.start <= k_range.start - (q_range.end - 1)
            && d_range.end >= k_range.end - 1 - q_range.start;
    }

    bool is_causal() const {
        return d_range.start <= k_range.start - (q_range.end - 1)
            && d_range.end == k_range.end - q_range.end;
    }

    bool is_inv_causal() const {
        return d_range.start == k_range.start - q_range.start
            && d_range.end >= k_range.end - 1 - q_range.start;
    }
    ...
    void append_qk_range_mask_type(
        ...
        if (q_id_l < q_range.start || q_id_l >= q_range.end
         || q_id_r < q_range.start || q_id_r >= q_range.end) {
            throw std::invalid_argument(
                "rect without shrinkage call to_qk_range_mask_type"
            );
        }
    ...
    int area() {
        return count_areas(
            ...
            d_range.start, d_range.end
        );
    }

    int count_areas(int lq, int rq, int lk, int rk, int ld, int rd) {
        if (rq <= lq || rk <= lk || rd < ld) return 0;
        ...
        if (a <= b) {
            // Interval 1
            int q_start = Q1;
            int q_end = std::min(a - 1, Q2);
            if (q_start <= q_end) {
                int m = std::max(q_start, K1 - rd);
                int n = q_end;
                if (m <= n) {
                    int c = rd - K1 + 1;
                    int sum_q = n * (n + 1) / 2 - (m - 1) * m / 2;
                    int sum_c = c * (n - m + 1);
                    total += sum_q + sum_c;
                }
            }
    ...
    bool operator==(const AttnRectangle& other) const {
        return q_range == other.q_range
            && k_range == other.k_range
