...
def native_grpcoll_split_alignment() -> int:
    ...
    used ONLY when enabling native group-collective communication kernels
    ...
    alignment = (
        os.environ.get("MAGI_ATTENTION_NATIVE_GRPCOLL_SPLIT_ALIGNMENT", "1")
        if is_native_grpcoll_enable()
        else "1"
    )
